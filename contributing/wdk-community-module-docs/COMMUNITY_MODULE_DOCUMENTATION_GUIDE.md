...
3. Inspect current rendered docs, `custom-tree.ts`, aggregate pages, collection
   configuration, validation scripts, and relevant open PRs.
...
| `content/docs/sdk/all-modules.mdx` | Package-oriented aggregate module list. |
| `content/docs/sdk/community-modules/index.mdx` | Community catalog, disclaimer, submission flow, and community rows. |
...
| Catalog-only integrations | Public package/source entry without a local page set | Community catalog only; do not invent a local route. |
...
a Community Modules page as a shortcut.
...
| Catalog-only integration | No local page set. Require a verified public package/source link, concise source-backed description, type, maintainer, and public destination. | Promote to local docs only when a reader journey justifies it. | `community-modules/index.mdx` and any aggregate list that intentionally includes catalog-only entries; no sidebar entry. |
| New-family candidate | Page set derived from the complexity overlay after maintainer IA approval. | Family index or chooser only when more than one module or a distinct decision journey exists. | Approved canonical path, relevant aggregate surfaces, and `custom-tree.ts` only for rendered discoverability. |
...
A catalog-only row must include a verified package or repository, module type,
one source-backed capability sentence, public documentation destination, and
maintainer identity. Verify that the destination actually explains installation
and API use. Do not create an empty local overview merely to obtain a docs URL.
...
| Swidge provider, any ownership | `content/docs/sdk/swidge-modules/<route>/` under `Swap and Bridge`; independently maintained providers are cross-listed in Community Modules. |
| Standalone swap, any ownership | `content/docs/sdk/swap-modules/<route>/` under `Swap and Bridge`; community ownership adds cross-listing, not a second canonical page. |
| Standalone bridge, any ownership | `content/docs/sdk/bridge-modules/<route>/` under `Swap and Bridge`; community ownership adds cross-listing, not a second canonical page. |
| Lending provider, any ownership | `content/docs/sdk/lending-modules/<route>/` under `Lending`; community ownership adds cross-listing. |
| Fiat provider, any ownership | `content/docs/sdk/fiat-modules/<route>/` under `On-ramp and Off-ramp`; community ownership adds cross-listing. |
...
| Catalog-only integration | No local module route or sidebar node; edit only the approved catalog/aggregate surfaces. |
...
- Update `all-modules.mdx` when the package is a module users should discover by package name.
- Update `community-modules/index.mdx` for independently maintained modules.
