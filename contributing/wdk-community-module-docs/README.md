...
rendered target only for `RENDERED`, the exact approved feed source for
`CATALOG_ONLY`, and no MDX for classification/no-route policies. That mode does
not compare baseline fragments or prove API semantics. MDX and base-comparison
modes require the repo-locked `github-slugger`; executable fences also require
TypeScript.
...
decision. `CATALOG_ONLY` means maintainers already approved an exact
`content/feeds/all-modules.md` edit but no module route or sidebar node.
`NO_APPROVED_ROUTE` means
...
Do not remove `content/feeds/all-modules.md`, rendered functional indexes,
adjacent examples, source configuration, or validation scripts. They are
legitimate current style and IA inputs.
...
containing exactly `content/feeds/all-modules.md`, copy it byte-for-byte to
...
(
  ...
  case "$FIXTURE_POLICY" in
    RENDERED)
      ...
      ;;
    CATALOG_ONLY)
      printf 'content/feeds/all-modules.md\n' \
        | cmp - "$HARNESS_DIR/allowed-outputs.txt"
      test -f content/feeds/all-modules.md
      test ! -L content/feeds/all-modules.md
      LINK_CHECK_EXTERNAL=false node scripts/check-links.mjs \
        > "$HARNESS_DIR/current-markdown.log" 2>&1
      ;;
    CLASSIFICATION_ONLY|NO_APPROVED_ROUTE)
      test -z "${TARGET_DOCS_DIR:-}"
...
run_logged 00-feed-sync npm run sync:all-modules-feed
