...
console.log('GasFree transfer ID:', transferResult.hash)
...
The returned `hash` is the GasFree transfer ID, not the on-chain TRON transaction ID. Pass it to [`getTransaction()`](/sdk/wallet-modules/wallet-tron-gasfree/api-reference#gettransactionhash) or [`waitForTransaction()`](/sdk/wallet-modules/wallet-tron-gasfree/api-reference#waitfortransactionhash-options); the native receipt exposes the chain transaction after the relay publishes it.

The returned `fee` includes the GasFree transfer fee and, when the GasFree account is not active yet, the token activation fee returned by the provider. Both `fee` and `activationFee` are returned as `bigint`; `activationFee` is `0n` when no activation fee applies.
...
The call rejects only when the quoted total fee is greater than `transferMaxFee`; a fee equal to the cap is allowed. The constructor-level `transferMaxFee` field is not used as a default by the current runtime, so pass the cap on every transfer that needs one.
