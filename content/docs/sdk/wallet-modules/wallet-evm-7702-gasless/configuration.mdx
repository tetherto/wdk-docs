...
const wallet = new WalletManagerEvm7702Gasless(seedPhrase, {
  ...
  delegationAddress: '<VERIFIED_DELEGATION_ADDRESS>',
  bundlerUrl: 'https://api.candide.dev/api/v3/1/YOUR_API_KEY',
  paymasterAddress: '0xa8151918eac3818deb713d3dbbb7930329fe86ed',
  paymasterToken: {
...
Sponsorship policies are created in the Candide dashboard. A public policy applies with no policy ID, while a private policy is referenced by `sponsorshipPolicyId`, so use an authenticated endpoint for your account's own policies and for higher rate limits.

### Using Pimlico

Pimlico is the tested alternative. A single URL serves both the bundler and the paymaster, with the API key in the query string:

```javascript title="Using Pimlico"
const wallet = new WalletManagerEvm7702Gasless(seedPhrase, {
  provider: 'https://rpc.mevblocker.io/fast',
  delegationAddress: '<VERIFIED_DELEGATION_ADDRESS>',
...
<Callout type="info">
`paymasterAddress` is specific to a provider, chain, and EntryPoint version. The addresses used on this page are the Ethereum mainnet token paymasters on EntryPoint v0.8: `0xa8151918eac3818deb713d3dbbb7930329fe86ed` for Candide and `0x888888888888Ec68A58AB8094Cc1AD20Ba3D2402` for Pimlico. Omit the field to accept the paymaster address the provider's RPC reports, or confirm the expected contract with your provider before pinning it.
</Callout>
