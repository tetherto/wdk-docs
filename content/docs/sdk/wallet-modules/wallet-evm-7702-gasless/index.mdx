...
- **Independent nonce lanes**: Use named or one-time EntryPoint lanes for concurrent operations that do not require ordering.
...
| Paymaster token | `paymasterToken.address`, optional `paymasterAddress`, optional `transactionMaxFee` and `transferMaxFee` | Quotes return a fee in the paymaster token's base units. |

`transactionMaxFee` applies when this wallet builds a non-sponsored send, signed UserOperation, or approval. It does not apply when `sendTransaction()` receives an already-signed UserOperation because that operation does not retain a reconstructible paymaster-token fee.
...
Review required fields, fee modes, provider failover, nonce lanes, and per-call overrides.
</Card>
...
Sign, quote, and submit ERC-4337 UserOperations, including concurrent sends on independent nonce lanes.
</Card>
</Cards>
