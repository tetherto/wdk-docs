...
| `signTransaction(tx)` | Signs a Tron transaction without broadcasting it | `Promise\<TronSignedTransaction\>` | If no provider, fee exceeds `transactionMaxFee`, or an unsigned pre-built transaction has inconsistent raw data or the wrong owner |
...
console.log('Current token allowance:', allowance)
```

##### `getTransactionReceipt(hash)`
Returns a native TRON receipt if it has been processed. This method is deprecated; use [`getTransaction()`](#gettransactionhash) for normalized finality and read its `receipt` field when you need the native object.
