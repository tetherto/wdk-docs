...
This guide explains how to [send EVM transactions with EIP-1559 gas parameters](#send-with-eip-1559-gas-parameters), [send legacy gas transactions](#send-with-legacy-gas-parameters), [deploy contracts](#deploy-a-contract), [sign without broadcasting](#sign-without-broadcasting), [estimate fees](#estimate-transaction-fees), [cap transaction fees](#cap-transaction-fees), [use dynamic fee rates](#use-dynamic-fee-rates), and [wait for finality](#wait-for-finality).
...
<Callout type="info">
`signTransaction()` returns signed raw transaction hex and does not broadcast it. In `1.0.0-beta.18`, `sendTransaction(signedTransaction)` checks `transactionMaxFee` against a provider-current gas and fee-rate estimate, then passes those exact bytes to `eth_sendRawTransaction`; it does not repopulate or re-sign them. The cap is not an upper bound on the gas price or maximum fee encoded in the signed transaction. Submit only after independently decoding and reviewing its gas limit and fee fields together with the recipient, value, data, chain ID, and nonce. Use the object form of `sendTransaction()` when WDK should populate, sign, and broadcast the transaction.
</Callout>
...
`reviewSignedTransaction` is an application-supplied callback. It must decode the signed transaction, show or otherwise verify its recipient, value, data, chain ID, nonce, gas limit, and fee fields, and return `{ approved: true }` only after explicit approval. Any other result fails closed without broadcasting.
...
```javascript title="Wait for a Finalized Transaction"
const result = await account.sendTransaction({
  to: recipient,
