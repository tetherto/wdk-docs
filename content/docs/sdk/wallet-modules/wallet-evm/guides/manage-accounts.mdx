...
<Callout type="info">
`PrivateKeySignerEvm` is not derivable. Use `getAccount('treasury')` to bind the named signer directly. In `1.0.0-beta.17`, attempts to call its `derive()` method throw `InvalidSignerError` from `@tetherto/wdk-wallet` rather than the earlier `SignerError` class.
</Callout>
