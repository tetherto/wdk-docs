...
const config = {
  ...
  // Optional: Maximum fee for native send/sign operations (in wei)
...
The `transactionMaxFee` option sets a maximum fee limit for native EVM `sendTransaction()` and `signTransaction()` operations. This is separate from `transferMaxFee`, which applies to ERC-20 token transfers.
