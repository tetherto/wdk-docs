...
In beta.19, the runtime also accepts an already-built ethers `Provider` and reuses one provider object across every account created by this manager. The published TypeScript declaration still exposes only the provider types shown above, so this runtime input is not yet a fully typed configuration path.
...
For a signed raw transaction string, WDK checks `transactionMaxFee` against a provider-current gas and fee-rate estimate, then passes the exact supplied hex to `eth_sendRawTransaction`. It does not repopulate or re-sign the transaction. The cap is not an upper bound on the gas price or maximum fee encoded in those signed bytes. Review the decoded recipient, value, data, chain ID, nonce, gas limit, and fee fields before submitting externally supplied bytes.
...
The declaration above is the exact beta.19 published type. The beta.19 runtime additionally accepts an ethers `Provider` directly or inside the provider array, but that input is missing from the package declaration.
