...
Starting in beta.15, the package root re-exports `AssertionError`, `MaximumFeeExceededError`, `NoSuchElementError`, `ProviderRequiredError`, `TimeoutError`, `ValueError`, and `WdkError` from `@tetherto/wdk-wallet`.
...
**Throws:** `ValueError` if a derivation-path segment is not hardened.
...
**Throws:** `ValueError` if a string seed is not a valid BIP-39 mnemonic or a derivation-path segment is not hardened.
...
**Throws:**
- `AssertionError` if the account has been disposed.
- `ProviderRequiredError` if the wallet is not connected to a provider.
- `MaximumFeeExceededError` if the estimated fee is greater than `transactionMaxFee`.
- `ValueError` if the transaction fee payer does not match the account or the provider cannot calculate the fee.
- A Solana error with code `SOLANA_ERROR__TRANSACTION__SIGNATURES_MISSING` if required signatures remain missing after this account signs a serialized transaction.
...
**Throws:**
- `ProviderRequiredError` if the wallet is not connected to a provider.
- `ValueError` if the fee payer does not match this account or the provider cannot calculate the fee.
...
- `AssertionError` if the account has been disposed.
- `ProviderRequiredError` if the wallet is not connected to a provider.
- `MaximumFeeExceededError` if the estimated fee is greater than `transferMaxFee`.
- `ValueError` if the token amount exceeds the supported integer range or the provider cannot calculate the fee.
...
**Throws:**
- `ProviderRequiredError` if the wallet is not connected to a provider.
- `ValueError` if `hash` is not a valid Solana signature.
...
console.log(receipt.transaction)   // Native transaction object, or null
```

Solana `processed` maps to `pending`, `confirmed` maps to `confirmed`, and `finalized` maps to `final`. The `block` field is the slot and `fee` is the native transaction fee when available. A settled transaction reports `success: false` when its status contains an error. The native `transaction` can still be `null` when the RPC does not return it at the configured commitment.

No configured provider throws `ProviderRequiredError`. An invalid signature throws `ValueError`; a well-formed signature absent from status history throws `NoSuchElementError`.
...
`waitForTransaction()` immediately propagates `ProviderRequiredError` and `ValueError`, treats `NoSuchElementError` as transient while the signature can still propagate, and throws `TimeoutError` if the target is not reached. The Solana implementation does not currently classify an evicted or never-landed signature as `dropped`. Reaching a finality target does not guarantee execution success, so inspect `success`.
...
Read-only accounts expose the same normalized signature-status methods as owned accounts. `getTransaction()` maps `processed`, `confirmed`, and `finalized` to `pending`, `confirmed`, and `final`; it throws `ProviderRequiredError` without a provider, `ValueError` for an invalid signature, and `NoSuchElementError` when a valid signature is absent from history. `waitForTransaction()` propagates the provider and input errors, treats an absent history entry as transient, and throws `TimeoutError` if the target is not reached within its default 60 seconds. Inspect `success` after settlement. `getTransactionReceipt()` remains available for the native object but is deprecated.
