...
const wallet = new WalletManagerBtc(seedPhrase, {
  ...
  network: 'bitcoin',
  transactionMaxFee: 10000n // Optional: maximum send/sign fee in satoshis
})
```
...
#### Unconfirmed Balance Handling

The selected client affects [`getBalance()`](/sdk/wallet-modules/wallet-btc/guides/check-balances) while transactions are pending:

- `blockbook-http` reports `unconfirmedOutgoing`. It subtracts address-owned confirmed inputs spent by pending transactions, credits pending change only when every input is address-owned and confirmed or descends from a trusted pending transaction, and excludes other unconfirmed incoming outputs.
- The built-in Electrum clients return only the server's raw `unconfirmed` delta, so the wallet adds that value to the confirmed balance.
- A custom `IBtcClient` can return optional `BtcBalance.unconfirmedOutgoing`. When present, the wallet subtracts it from `confirmed` and does not use `unconfirmed`.

Use the same client policy anywhere balances are compared or displayed. Blockbook and Electrum results can differ until pending transactions confirm.
...
In `1.0.0-beta.14`, `IBtcClient` adds the required `getBlockHeight(): Promise<number>` method. The built-in Electrum and Blockbook clients implement it. Update custom TypeScript implementations before upgrading. At runtime, a legacy custom client without this method can still read a normalized included transaction, but `confirmations` is `null` and its finality cannot advance from `confirmed` to `final` because the chain tip is unknown.
...
const wallet = new WalletManagerBtc(seedPhrase, {
  network: 'bitcoin',
  transactionMaxFee: 10000n // 10,000 satoshis
})
```

Use [`quoteSendTransaction()`](/sdk/wallet-modules/wallet-btc/api-reference#quotesendtransactionoptions) when you want to show the estimated fee before calling `sendTransaction()`.
