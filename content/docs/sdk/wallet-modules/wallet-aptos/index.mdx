...
- **Normalized transaction tracking**: Look up or wait for transaction finality with `getTransaction()` and `waitForTransaction()` while retaining the raw Aptos transaction.
...
| Transaction finality | Mempool transactions are `pending`; committed transactions are `final`. A final receipt can still report `success: false`. |
...
<Callout type="warn">
Native `sendTransaction()` and `signTransaction()` do not apply `transferMaxFee`. A prior quote is advisory because those operations simulate again and expose no native maximum-fee argument. Beta.2 cannot enforce a hard application cap for native APT operations.
</Callout>
