...
Returns a transaction's native TON receipt if it has been included in a block. This method is deprecated; use [`getTransaction()`](#gettransactionhash) for normalized finality and read its `transaction` field when you need the native object.
