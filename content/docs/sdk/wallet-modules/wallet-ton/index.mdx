...
- **Normalized Transaction Status**: Read or wait for the message-body hash returned by sends, with TON `confirmed` and `final` states and the native transaction attached
- **Signed Body Submission**: Quote and send the signed transfer-body `Cell` returned by `signTransaction()`
