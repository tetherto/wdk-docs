...
If HD-key derivation cannot produce the master key's private or public key, the wallet raises the Spark SDK `ValidationError` with `field: 'hdkey'`. If a derived child key is incomplete, it uses `field: 'derivedKeys'`. In beta.26 these derivation errors do not include seed bytes in their context; validation errors elsewhere may still contain sensitive context.
...
| `getTransaction(hash)` | Returns a normalized receipt for a Spark transfer ID | `Promise<TransactionReceipt & SparkTransactionDetails>` |
| `waitForTransaction(hash, options?)` | Waits for a Spark transfer to reach the requested finality | `Promise<TransactionReceipt & SparkTransactionDetails>` |
| `getTransactionReceipt(hash)` | Deprecated: returns a Spark transfer by its ID | `Promise<SparkTransfer \| null>` |
...
Returns a Spark transfer by its ID. This method is deprecated; use [`getTransaction()`](#gettransactionhash) for normalized status and keep using the returned `transfer` field when you need the native Spark object. Neither method looks up on-chain Bitcoin transactions.
...
##### `getTransaction(hash)`
Returns a normalized receipt for a Spark transfer ID. A transfer with status `COMPLETED` is `final` with `success: true`; `EXPIRED` and `RETURNED` transfers are `dropped`; every other Spark transfer status is `pending`. The receipt has `fee: 0n` and includes the native Spark transfer on `transfer`.

**Parameters:**
- `hash` (string): The Spark transfer ID returned by `sendTransaction()` or `transfer()`

**Returns:** `Promise<TransactionReceipt & SparkTransactionDetails>`

**Throws:** `NoSuchElementError` if Spark does not return a transfer for the ID.

```javascript
const result = await account.sendTransaction({ to: 'spark1...', value: 1000n })
const transaction = await account.getTransaction(result.hash)

console.log(transaction.finality) // 'pending', 'final', or 'dropped'
console.log(transaction.transfer) // Native Spark transfer
```

##### `waitForTransaction(hash, options?)`
Polls [`getTransaction()`](#gettransactionhash) until the requested target is reached, a dropped transfer is observed twice, or the wait times out. Defaults are `target: 'confirmed'`, `interval: 4000`, `timeout: 60000`, and `maxPollErrors: 3`.
...
if (transaction.finality === 'dropped') {
  console.error('Spark transfer expired or was returned')
}
```
...
| `keyPair` | `KeyPair` | The account's key pair (⚠️ Contains sensitive data). The returned arrays are bound to the account — treat them as a read-only view and do not modify their contents. `privateKey` is `null` after `dispose()` is called. |
...
##### `getTransactionReceipt(hash)`
Returns a Spark transfer by its ID. This method is deprecated in favor of `getTransaction()`. The read-only account's `getTransaction()` and `waitForTransaction()` use the same Spark transfer-ID, finality, timeout, and raw `transfer` semantics documented above. These methods do not look up on-chain Bitcoin transactions.
...
interface KeyPair {
  ...
  privateKey: Uint8Array | null  // Private key bytes; null after dispose() is called
}
```
