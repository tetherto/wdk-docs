...
<Callout type="info">
These pages reflect `@tetherto/wdk-wallet-spark@1.0.0-beta.26`. This release removes seed bytes from HD-key derivation error context while retaining the `hdkey` field identifier; validation-error context remains sensitive.
</Callout>
...
- **Normalized Transfer Status**: Read or wait for Spark transfer IDs with `pending`, `final`, and `dropped` finality and the native Spark transfer attached
