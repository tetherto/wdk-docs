...
<Callout type="warning">
Starting in `@tetherto/wdk-wallet-ton-gasless` `1.0.0-beta.11`, `getTransaction()` and the deprecated `getTransactionReceipt()` inherit TON beta.13's configured-origin lookup, so a testnet `tonClient` uses the testnet TON Center v3 origin. `getFeeRates()` still reads mainnet TON API configuration; do not use it as a testnet-specific fee-rate source.
</Callout>
...
- Treat a relay-estimate validation error as a failed preflight. Do not sign or submit a replacement message assembled from unvalidated relay fields
