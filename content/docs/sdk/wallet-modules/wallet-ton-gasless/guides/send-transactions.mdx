...
You can retrieve mainnet TON API fee rates from the wallet manager using [`wallet.getFeeRates()`](/sdk/wallet-modules/wallet-ton-gasless/api-reference#getfeerates). In `1.0.0-beta.11`, this method still does not follow configured testnet clients and returns the same calculated value for `normal` and `fast`:
