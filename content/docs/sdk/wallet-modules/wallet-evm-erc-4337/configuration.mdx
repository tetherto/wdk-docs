...
The `bundlerHeaders` option sets HTTP headers sent on every request to the bundler. Use it for bundlers that authenticate with a header (e.g. `Authorization: Bearer <key>`). When omitted, no extra headers are sent, so bundlers that authenticate via the URL keep working unchanged. Available starting in `v1.0.0-beta.20`.
...
The `paymasterHeaders` option sets HTTP headers sent on every request to the paymaster. Use it for paymasters that authenticate with a header (e.g. `Authorization: Bearer <key>`). When omitted, no extra headers are sent, so paymasters that authenticate via the URL keep working unchanged. Available starting in `v1.0.0-beta.20`.
...
A matching quote can be cached for up to two minutes by transaction data. The cache key does not include per-call fee-mode or paymaster settings, and reusing the cached UserOperation does not call the paymaster RPC or rerun this address check. Keep `isSponsored`, `useNativeCoins`, `paymasterUrl`, `paymasterHeaders`, `paymasterAddress`, `paymasterToken`, and `sponsorshipPolicyId` consistent between a quote and its matching sign or send operation.
...
```javascript title="Configure A Named Lane After Safe Deployment"
const config = {
...
The beta.20 runtime also honors `parallel` and `nonceKey` in the per-call config for `signTransaction()`, `sendTransaction()`, and `transfer()`, but the published per-call TypeScript declarations omit both fields. Construction-level configuration is typed; JavaScript per-call usage works at runtime.
</Callout>

### Safe Modules Version

The `safeModulesVersion` option specifies the Safe modules version for smart contract wallet implementation. Beta.20 supports only `0.3.0`; other values, including `0.2.0`, throw `ConfigurationError`. **Required** for smart account initialization.
...
[Openfort](https://www.openfort.io) serves an ERC-4337 bundler and an ERC-7677 paymaster from a single RPC endpoint per chain, authenticated with your project's publishable key as a bearer token. The same URL and header serve as both `bundlerUrl` and `paymasterUrl`.
...
<Callout type="warn">
Two provider-side details decide whether a sponsored operation goes through:

- **EntryPoint version.** This module creates accounts on EntryPoint v0.7. A bundler that serves only other versions rejects the operation with `-32602 Unsupported EntryPoint`. Openfort currently serves v0.6, v0.8 and v0.9, so operations from this module are rejected at that check; confirm your provider serves v0.7 before using it here.
- **Sponsorship context key.** In sponsorship mode the module sends the policy as `sponsorshipPolicyId` in the ERC-7677 context. Openfort's paymaster expects `policyId` and rejects unknown keys (`Unrecognized key: "sponsorshipPolicyId"`), so leave `sponsorshipPolicyId` unset with this provider.
</Callout>
