...
In beta.20, the wallet classifies an AbstractionKit error as AA50 from its structured `aaCode` first, with message matching as a compatibility fallback. The same `TransactionError` mapping applies when AA50 occurs during fee estimation, UserOperation building, or bundler submission.
...
- [Configuration](/sdk/wallet-modules/wallet-evm-erc-4337/configuration) - Configure bundler, paymaster, and fee limits.
- [API Reference](/sdk/wallet-modules/wallet-evm-erc-4337/api-reference) - Check method signatures and error types.
