...
## Wait for UserOperation Finality

The `hash` returned by `sendTransaction()` is a UserOperation hash, not the hash of the EVM transaction that later bundles it. Pass that UserOperation hash to `getTransaction()` or `waitForTransaction()`:

```javascript title="Wait for UserOperation Finality"
const receipt = await account.waitForTransaction(result.hash, {
  target: 'final'
})

console.log('UserOperation:', receipt.hash)
console.log('Finality:', receipt.finality)
console.log('Bundling receipt:', receipt.receipt)
...
Before an EVM transaction hash is assigned, `getTransaction()` returns `pending`. Once included, finality and confirmations come from the bundling transaction; `success` and `fee` use the UserOperation receipt's values when available. The default wait interval is four seconds and the timeout is 180 seconds.

<Callout type="info">
`getTransactionReceipt()` remains available for the native EVM receipt but is deprecated. It also expects the UserOperation hash. Use `getTransaction()` for normalized finality and access `receipt` or `userOperationReceipt` for native data.
</Callout>
...
<Callout type="warning">
Cache identity does not include per-call fee-mode configuration. Use the same fee mode, `paymasterUrl`, `paymasterHeaders`, paymaster address and token, and sponsorship policy for `quoteSendTransaction()` and the matching `sendTransaction()` or `signTransaction()`. Do not change one of these settings while the quote is cached.
</Callout>
...
`signTransaction()` preserves the lane chosen by the same configuration rules. In beta.20, per-call `parallel` and `nonceKey` work at runtime but are missing from the published per-call TypeScript declarations; construction-level lane configuration is typed.
...
When switching from sponsored or native-coin mode, also set `isSponsored: false` and `useNativeCoins: false`, and provide any paymaster fields absent from the account configuration. `confirmFeeTokenAndAmount()` is an application-owned confirmation step. Keep the selected token and `transactionMaxFee` in the same reviewed override; quoting displays the fee but does not enforce the cap.
