...
This guide explains how to [transfer ERC-20 tokens](#transfer-erc-20-tokens), [estimate transfer fees](#estimate-transfer-fees), [set a maximum fee limit](#transfer-with-maximum-fee-limit), [select a nonce lane](#transfer-in-a-nonce-lane), and [apply UserOperation gas overrides](#transfer-with-useroperation-gas-overrides).
...
Set UserOperation gas or EIP-1559 fee overrides directly on the options object passed to `transfer()` or `quoteTransfer()`, alongside `token`, `recipient` and `amount`. Set `maxFeePerGas` and `maxPriorityFeePerGas` together.
...
`approve()` accepts the same override fields on its options object.
