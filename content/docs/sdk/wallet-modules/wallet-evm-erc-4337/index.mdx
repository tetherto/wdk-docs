...
description: Create EVM smart accounts that submit UserOperations through ERC-4337 bundler and paymaster infrastructure.
docType: explanation
...
<Callout type="info">
These pages reflect `@tetherto/wdk-wallet-evm-erc-4337@1.0.0-beta.20`. This release adds authenticated bundler and paymaster request headers; AA50 prefund failures map to `TransactionErrorReason.INSUFFICIENT_BALANCE`.
</Callout>
...
- **Normalized UserOperation Status**: Track a UserOperation from bundler acceptance through its EVM transaction's confirmed or final state
- **Separated Submission Flow**: Sign one UserOperation, review or quote it, then submit the same operation
- **Parallel Nonce Lanes**: Opt into independent ERC-4337 nonce keys for concurrent UserOperations
