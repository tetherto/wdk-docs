...
**Optional common config fields:**
  ...
  - `nonceKey` (number | bigint | string): Reuse a raw uint192 key or a deterministic named lane; takes precedence over `parallel`
...
| `transfer(options, config?)` | Transfers ERC20 tokens via UserOperation | `Promise\<{hash: string, fee: bigint}\>` | If fee exceeds max, or the token paymaster address mismatches |
| `quoteTransfer(options, config?)` | Estimates the fee for an ERC20 transfer | `Promise\<{fee: bigint}\>` | If the token paymaster address mismatches |
| `approve(options)` | Approves a spender to spend ERC20 tokens | `Promise\<{hash: string, fee: bigint}\>` | `ProviderRequiredError`, or `ValueError` when Ethereum USD₮ allowance must be reset |
...
**Parameters:**
- `tx` (EvmErc4337Transaction): One transaction. Batch arrays are not accepted by this method.
...
**Parameters:**
- `tx` (EvmErc4337Transaction | EvmErc4337Transaction[] | UserOperationV7): Transaction object, array for batch transactions, or a signed UserOperation
...
This quote method does not resolve or reserve a nonce lane. A later send or sign using `parallel` or `nonceKey` rebuilds in that lane instead of reusing the quote.

<Callout type="warning">
The cache key contains the transaction but not the per-call fee-mode configuration. Use the same fee-mode and paymaster settings for a quote and its matching send, sign, or transfer. Do not quote under one mode, token, paymaster, or sponsorship policy and execute the same transaction under another while the quote is cached.
</Callout>

**Parameters:**
- `tx` (EvmErc4337Transaction | EvmErc4337Transaction[] | UserOperationV7): Transaction object, array, or signed UserOperation
- `config` (optional): Per-call configuration override (see [Config Override](#config-override))
...
**Parameters:**
- `options` (EvmErc4337TransferOptions): Transfer options
  - `token` (string): ERC20 token contract address
  - `recipient` (string): Recipient address
  - `amount` (number | bigint): Amount in token base units
  - Optional UserOperation gas and fee overrides (see [EvmErc4337GasOverrides](#evmerc4337gasoverrides))
- `config` (optional): Per-call configuration override (see [Config Override](#config-override))
...
**Parameters:**
- `options` (EvmErc4337TransferOptions): Transfer options (same as transfer), including any optional UserOperation gas and fee overrides (see [EvmErc4337GasOverrides](#evmerc4337gasoverrides))
- `config` (optional): Per-call configuration override (see [Config Override](#config-override))
...
**Parameters:**
- `options` (EvmErc4337ApproveOptions): Approve options
  - `token` (string): ERC20 token contract address
...
**Parameters:**
- `hash` (string): The UserOperation hash, not the EVM bundling-transaction hash
...
```javascript
const receipt = await account.getTransaction(userOperationHash)

console.log(receipt.hash)                 // The UserOperation hash
console.log(receipt.finality)             // pending, confirmed, final, or dropped
console.log(receipt.success)              // UserOperation execution result when known
console.log(receipt.fee)                  // UserOperation actualGasCost when known
console.log(receipt.receipt)              // Native EVM bundling-transaction receipt
console.log(receipt.userOperationReceipt) // Raw bundler receipt
```
...
| `quoteTransfer(options, config?)` | Estimates the fee for an ERC20 transfer | `Promise\<{fee: bigint}\>` | If simulation fails or the token paymaster address mismatches |
| `getTransactionReceipt(hash)` | Maps a UserOperation hash to a native EVM receipt; deprecated in favor of `getTransaction()` | `Promise\<EvmTransactionReceipt \| null\>` | - |
| `getTransaction(hash)` | Returns normalized UserOperation finality using its bundling transaction | `Promise\<TransactionReceipt & EvmErc4337TransactionDetails\>` | If the hash is invalid or unknown |
| `waitForTransaction(hash, options?)` | Waits for the UserOperation to reach `confirmed` or `final`, or returns `dropped` | `Promise\<TransactionReceipt & EvmErc4337TransactionDetails\>` | If the wait times out |
| `getUserOperationReceipt(hash)` | Returns a UserOperation receipt | `Promise\<UserOperationReceipt \| null\>` | - |
...
**Parameters:**
- `options` (EvmErc4337TransferOptions): Transfer options, including any optional UserOperation gas and fee overrides (see [EvmErc4337GasOverrides](#evmerc4337gasoverrides))
- `config` (optional): Per-call configuration override (see [Config Override](#config-override))
...
interface WaitForTransactionOptions {
  ...
  timeout?: number                  // Milliseconds; ERC-4337 default: 180,000
  interval?: number                 // Milliseconds; default: 4,000
  maxPollErrors?: number            // Default: 3
}

interface EvmErc4337TransactionDetails {
...
- `isSponsored` (boolean): Enable or disable sponsorship mode
- `useNativeCoins` (boolean): Enable or disable native-coin mode
...
`UserOperationV7` is the ERC-4337 v0.7 operation type re-exported by `@tetherto/wdk-wallet-evm-erc-4337` from AbstractionKit.

```typescript
import type { UserOperationV7 } from '@tetherto/wdk-wallet-evm-erc-4337'
```

`signTransaction()` returns this type with its `signature` populated. Passing that signed value to `quoteSendTransaction()` estimates its submitted cost; passing it to `sendTransaction()` submits the same operation without rebuilding it.

### EvmErc4337GasOverrides

The helper methods `transfer()`, `quoteTransfer()`, and `approve()` accept these same UserOperation gas and fee override fields directly on their options object; see [EvmErc4337TransferOptions](#evmerc4337transferoptions) and [EvmErc4337ApproveOptions](#evmerc4337approveoptions). Set `maxFeePerGas` and `maxPriorityFeePerGas` together.
...
### EvmErc4337TransferOptions

The options accepted by `transfer()` and `quoteTransfer()`: the standard [TransferOptions](#transferoptions) extended with the optional [EvmErc4337GasOverrides](#evmerc4337gasoverrides) fields.

```typescript
type EvmErc4337TransferOptions = TransferOptions & EvmErc4337GasOverrides
```

### EvmErc4337ApproveOptions

The options accepted by `approve()`: the standard [ApproveOptions](#approveoptions) extended with the optional [EvmErc4337GasOverrides](#evmerc4337gasoverrides) fields.

```typescript
type EvmErc4337ApproveOptions = ApproveOptions & EvmErc4337GasOverrides
```
