...
**Common config fields (required for all modes):**
  ...
  - `safeModulesVersion` (string): Must be `'0.3.0'`, the only Safe modules version supported by beta.20

**Optional common config fields:**
  - `bundlerHeaders` (Record\<string, string\>): HTTP headers sent with every bundler request
...
- `paymasterHeaders` (Record\<string, string\>, optional): HTTP headers sent with every paymaster request
...
<Callout type="warning">
The cache key contains the transaction but not the per-call fee-mode configuration. Keep the same fee mode, paymaster URL, `paymasterHeaders`, paymaster address and token, and sponsorship policy for a quote and its matching send, sign, or transfer. Do not change one of these settings while the quote is cached.
</Callout>
...
interface EvmErc4337WalletCommonConfig {
  ...
  bundlerHeaders?: Record<string, string>;   // Optional headers sent to the bundler
  safeModulesVersion: string;               // Must be '0.3.0' in beta.20
...
interface EvmErc4337WalletPaymasterTokenConfig {
  ...
  paymasterHeaders?: Record<string, string>; // Optional headers sent to the paymaster
...
The published beta.20 `config` parameter type on `sendTransaction`, `signTransaction`, `quoteSendTransaction`, `transfer`, and `quoteTransfer` allows per-call overrides of gas payment settings:
...
- `paymasterHeaders` (`Record<string, string>`): Override paymaster request headers
...
At runtime, beta.20 additionally honors `parallel` and `nonceKey` per call for `signTransaction()`, `sendTransaction()`, and `transfer()`. Those two fields are missing from the published per-call declarations, so object literals containing them fail TypeScript excess-property checks even though JavaScript calls work. Construction-level `EvmErc4337WalletCommonConfig` includes both fields and is the typed path in this release. `quoteSendTransaction()` and `quoteTransfer()` do not select a lane.
...
The quote cache does not include fee-mode override fields in its key. Keep the same fee mode, paymaster URL, `paymasterHeaders`, paymaster address and token, and sponsorship policy for a quote and its matching send, sign, or transfer. Lane sends, signs, and transfers bypass cached UserOperations and rebuild in the selected lane.
