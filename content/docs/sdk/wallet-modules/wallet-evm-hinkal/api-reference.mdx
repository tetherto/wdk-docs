...
|---|---|---|---|
| `token` | `string` | Yes | ERC-20 address, or `0x0000000000000000000000000000000000000000` for the native token. Hinkal must support the chain and token. |
| `recipient` | `string` | Yes | Valid EVM destination address. |
| `amount` | `number` or `bigint` | Yes | Positive integer amount in the selected ERC-20 or native token's base units. Prefer `bigint`. |
...
For ERC-20 tokens, the SDK can submit approvals before the deposit. For the native token, it submits the deposit through transaction `value` and skips approval. Either deposit can precede later proof or scheduling failures. Deposit calculation adds a flat token fee to the supplied amount, including for native-token deposits; reserve native gas separately. There is no module-level private-send quote, fee cap, net-receive guarantee, or idempotency key. Inherited EVM fee caps do not apply.
...
The Hinkal SDK in this release defines these item statuses: `pending`, `processing`, `waiting_for_relayer`, `sent_on_chain`, `completed`, `failed`, and `deposit_failed`. The module still declares `status` as `string`; handle unknown values and each transaction item independently. These names do not establish a guaranteed sequence.
...
Two inherited constructors/conversions produce plain EVM accounts: `WalletAccountEvmHinkal.fromPrivateKey()` returns `WalletAccountEvm` synchronously, while `account.toReadOnlyAccount()` returns `Promise<WalletAccountReadOnlyEvm>` and resolves to the read-only account. Use `await account.toReadOnlyAccount()` to obtain it. Neither account has Hinkal methods. The package does not offer private-key or address-only alternatives to seed-based Hinkal account creation.
