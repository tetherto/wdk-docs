...
const wethUsd = await client.getCurrentPrice('WETH', 'USD')
```

Supplying a key that already exists in the built-in map replaces that symbol's default CoinGecko ID. Validate any replacement against CoinGecko before using it in production.
