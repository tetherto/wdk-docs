...
For a compatible protocol, use `wdk.registerProtocol(blockchain, label, ProtocolClass, config)` and retrieve it from an account with its typed getter, such as `getSwapProtocol(label)`. See [Protocol Integration](/sdk/core-module/guides/protocol-integration) for setup and execution examples.

<Callout type="warn">
Velora beta.8, USD₮0 bridge beta.10, and Aave beta.7 cannot consume the shared ethers provider on EVM wallet beta.19 manager accounts. Use the direct-account setup in the linked guide, and retain any application checks normally supplied by WDK policies and middleware.
</Callout>

#### Unified Operations

Registered wallet modules expose common account operations:
