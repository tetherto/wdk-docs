...
Both [`bridge()`](/sdk/bridge-modules/bridge-usdt0-evm/api-reference#bridgeoptions-config) and [`quoteBridge()`](/sdk/bridge-modules/bridge-usdt0-evm/api-reference#quotebridgeoptions-config) require a provider, valid route options, and a recipient accepted for the target ecosystem. Beta.10 rejects malformed and zero recipients, TRON's zero/burn address, and TON workchains other than `0`. Only execution rejects an account without a callable `sendTransaction()` method and enforces [`bridgeMaxFee`](/sdk/bridge-modules/bridge-usdt0-evm/api-reference#bridgeprotocolconfig). Read-only accounts can quote; a quote does not enforce the cap. For auto-resolved routes, the source and destination must differ. The [API reference](/sdk/bridge-modules/bridge-usdt0-evm/api-reference#error-handling) lists representative `error.message` substrings.
...
try {
  ...
  if (error.message.includes('Invalid recipient address for target chain')) {
    console.error('Recipient is invalid for the selected destination')
  }
}
```
