...
The example uses the exact Ethereum USD₮ and Arbitrum USD₮0 contract addresses. Resolve a different route through token discovery instead of reusing either address on another chain.
...
After the user confirms the quote, call `swidge()` with the same route shape. The module handles required ERC-20 approvals, including reset-to-zero flows for tokens such as USD₮ on Ethereum.
