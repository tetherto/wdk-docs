...
EVM token sources need token contract addresses. The package includes common Tether-family source addresses, but production wallets should pass their own allowlist.

```javascript title="Configure Arbitrum USDt0 source token"
...
Spark tokens other than BTC need Spark token identifiers. Replace the placeholder key and value below with the exact provider-returned symbol and identifier:
