...
Returns the static chain list bundled with version `0.1.0`.

Consume the returned `SwidgeSupportedChain[]` when building a chain selector instead of copying the inventory into application code. Each entry contains the chain id, display name, and native-token symbol for one network supported by the pinned package.
