...
[Prepare an EVM account](#prepare-an-evm-account), [confirm the operation](#confirm-the-operation), [submit the deposit](#submit-the-deposit), and [track settlement](#track-settlement). Use [recovery guidance](#recover-from-an-interrupted-operation) if an operation is interrupted. For support, see [Need Help?](#need-help).
...
See the complete [error reference](/sdk/swidge-modules/swidge-moonpay-trade/api-reference#errors). After completing account work, follow the wallet's [`dispose()` guidance](/sdk/wallet-modules/wallet-evm/guides/error-handling#dispose-of-sensitive-data). The protocol itself does not expose a disposal method.
