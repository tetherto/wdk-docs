...
Import the class and error classes from the package root in JavaScript, including `.mjs` files:

```javascript title="JavaScript imports"
...
For TypeScript projects, import types separately:

```typescript title="TypeScript type imports"
import type {
  MoonPayTradeConfig,
  MoonPayTradeSwidgeOptions,
  SwidgeReadOnlyAccount,
  SwidgeWritableAccount,
  ISwidgeProtocol
} from '@moonpay/wdk-protocol-swidge-moonpay-trade'
```
...
| `WalletAccountBtc` (native BTC only; token `transfer()` always throws) | `@tetherto/wdk-wallet-btc` |
