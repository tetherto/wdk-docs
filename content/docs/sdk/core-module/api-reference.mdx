...
`ISwidgeProtocol` is the shared interface for providers that can combine token swapping and cross-chain bridging in one route. See [Swidge Protocol Interface](/sdk/swidge-modules) for the full option, quote, result, fee, and status shapes.
