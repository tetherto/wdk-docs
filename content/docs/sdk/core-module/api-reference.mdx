...
`ISwidgeProtocol` is the shared interface for providers that can execute swap-only, bridge-only, or combined swap and bridge routes. It also exposes read-only discovery methods for supported chains and tokens. See [Swidge Protocol Interface](/sdk/swidge-modules) for the full option, quote, result, fee, status, and discovery shapes.
