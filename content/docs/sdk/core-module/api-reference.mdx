...
| [IWalletAccount](#iwalletaccount) | Base single-signer writable wallet account interface from `@tetherto/wdk-wallet`. | [Methods](#methods-1) |
| [Multisig account contracts](#multisig-account-contracts) | Implementation contracts exported from `@tetherto/wdk-wallet/multisig`. | Interfaces and types |
...
**Type Parameters:**
- `P`: `typeof SwapProtocol | typeof BridgeProtocol | typeof LendingProtocol | typeof FiatProtocol | typeof SwidgeProtocol | typeof SdaProtocol` - A class that extends one of the `@tetherto/wdk-wallet/protocols` classes

**Parameters:**
- `blockchain` (string): The name of the blockchain
- `label` (string): Registry label for the protocol. Registering the same blockchain, protocol type, and label again replaces the previous global registration.
...
<Callout type="warn">
Do not register Velora beta.8, USD₮0 bridge beta.10, or Aave beta.7 for accounts derived by `WalletManagerEvm` beta.19. Each protocol treats the manager's shared ethers `Provider` as EIP-1193 and rejects it during construction. Use a directly constructed `WalletAccountEvm` with the original RPC URL or EIP-1193 provider for these exact combinations, and never mutate the account's internal provider field. Direct accounts do not receive WDK policy or middleware decoration, so retain the required application checks and account-level controls.
</Callout>

**Example:**
```javascript title="Register Protocols"
// Register an illustrative swap provider for Ethereum
wdk.registerProtocol('ethereum', 'swap', MySwapProtocol, swapProtocolConfig)
...
`MySwapProtocol` is an illustrative class that extends `SwapProtocol`; use a concrete provider whose constructor supports the account and provider shape supplied by your wallet.
...
import WDK, { DENIAL_CODES, PolicyViolationError } from '@tetherto/wdk'
...
if (simulation.decision === 'DENY') {
  console.warn(simulation.code, simulation.reason)
}
...
try {
  ...
  if (error instanceof PolicyViolationError) {
    if (error.code === DENIAL_CODES.NO_APPLICABLE_RULE) {
      console.error(`No rule addresses ${error.operation}`)
    } else {
...
await presentSeedPhraseForSecureBackup(seedPhrase12)

// Generate 24-word seed phrase (higher security)
const seedPhrase24 = WDK.getRandomSeedPhrase(24)
await presentSeedPhraseForSecureBackup(seedPhrase24)
```
...
`DENIAL_CODES` is a root export with three machine-readable policy outcomes:

| Value | Meaning |
| --- | --- |
| `RULE_DENIED` | A matching `DENY` rule blocked the operation. |
| `NO_APPLICABLE_RULE` | No registered rule addressed the governed operation. |
| `GOVERNED_BUT_UNMATCHED` | Rules addressed the operation, but none of their condition sets matched. |

`PolicyViolationError.code` contains one of these values, and `PolicyViolationError.operation` names the blocked method. Compare `code` instead of parsing `reason`, because a matching rule can supply application-owned reason text. A simulation returns the same denial code without executing the governed method.
...
Treat `signedTransaction` as a sensitive, potentially broadcastable authorization payload. Do not log it; pass it only to the reviewed storage, transport, or submission path that needs it.
...
`getTransactionReceipt()` is marked deprecated in the base package. Current first-party Bitcoin, standard EVM, ERC-4337, EIP-7702, Solana, Solana Gasless, Spark, TON, TON Gasless, TRON, TRON Gasfree, and Aptos wallet releases implement `getTransaction()` and inherit `waitForTransaction()`. Use the concrete module's extended `TransactionReceipt` when you also need chain-native receipt fields.

<Callout type="info">
The base class still throws `NotImplementedError` when a third-party or older wallet module does not override `getTransaction()`. Confirm support in the concrete module before relying on normalized transaction tracking.
</Callout>
...
```javascript title="Import Multisig Contracts"
import {
...
These runtime exports are interface base classes. Their methods throw `NotImplementedError` until a wallet module overrides them.
...
```typescript title="Writable Multisig Contract"
...
```typescript title="Owner Management Contract"
...
```typescript title="Multisig Result Types"
...
**Parameters:**
- `label` (string): Registry label for the protocol. Registering the same protocol type and label again replaces the account-scoped instance.
- `protocol` (P): The protocol class
- `config` (`ConstructorParameters<P>[1]`): The protocol configuration
...
const account2 = (await wdk.getAccount('ethereum', 1))
  .registerProtocol('deposits', MySdaProtocol, sdaProtocolConfig)
```

`MySdaProtocol` is an illustrative class that extends `SdaProtocol`; replace it with the concrete provider used by your application.
...
account.registerProtocol('swap', MySwapProtocol, swapProtocolConfig)

// Get swap protocol
const swap = account.getSwapProtocol('swap')
...
`MySwapProtocol` is an illustrative class that extends `SwapProtocol`.
...
account.registerProtocol('bridge', MyBridgeProtocol, bridgeProtocolConfig)

// Get bridge protocol
const bridge = account.getBridgeProtocol('bridge')

// Use the protocol
const bridgeResult = await bridge.bridge({
...
`MyBridgeProtocol` is an illustrative class that extends `BridgeProtocol`. Follow the concrete provider's approval and configuration requirements before execution.
...
account.registerProtocol('lending', MyLendingProtocol, lendingProtocolConfig)

// Get lending protocol
const lending = account.getLendingProtocol('lending')

// Use the protocol
const supplyResult = await lending.supply({
  token: '0x...',
  amount: 1000000n
})
```

`MyLendingProtocol` is an illustrative class that extends `LendingProtocol`.
...
**Example:**
```javascript title="Register And Retrieve An SDA Protocol"
const account = await wdk.getAccount('ethereum', 0)

account.registerProtocol('deposits', MySdaProtocol, sdaProtocolConfig)

const deposits = account.getSdaProtocol('deposits')
const routes = await deposits.getSupportedRoutes({ outputAsset: 'USDT' })
```

Global SDA registrations are constructed when retrieved. Account-scoped SDA registrations are constructed when registered. If both scopes use the same label, the global provider is returned.
...
// Use protocols
const swap = accountEth.getSwapProtocol('swap')
const swapResult = await swap.swap(swapOptions)

// Clean up
wdk.dispose()
```

`MySwapProtocol` is illustrative and must support the manager account's provider shape. Velora beta.8, USD₮0 bridge beta.10, and Aave beta.7 are intentionally excluded from this manager-derived beta.19 flow. Their direct standard-account guides cover [Velora setup](/sdk/swap-modules/swap-velora-evm/guides/get-started), [bridge setup](/sdk/bridge-modules/bridge-usdt0-evm/guides/get-started), and [Aave setup](/sdk/lending-modules/lending-aave-evm/guides/get-started). Preserve application checks and account-level controls because WDK policies and middleware do not decorate those separate accounts.
...
Simulation results returned by the runtime `account.simulate.<method>(...)` mirrors include `decision`, `code`, `policy_id`, `matched_rule`, `reason`, and a `trace` array that records evaluated rules. `code` is `null` for an allowed call and matches the `PolicyViolationError.code` that execution would produce for a denied call. `reason` can include application-owned rule text or one of the engine outcomes, so branch on `code` when handling denials.
...
See the provider catalog in [Swidge modules](/sdk/swidge-modules), then use the selected provider's API reference for its released discovery, quote, execution, status, fee, and result behavior.
