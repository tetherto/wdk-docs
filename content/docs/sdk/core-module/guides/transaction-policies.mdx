...
Use `@tetherto/wdk` v1.0.0-beta.18 or later for these examples because they import `DENIAL_CODES` and read the stable `error.code` and `error.operation` fields. Method exclusions and coverage beyond the earlier fixed operation list arrived in beta.17. The examples assume you have loaded an app-owned `seedPhrase` and registered the relevant wallet modules.
...
try {
  ...
  if (error instanceof PolicyViolationError) {
    if (error.code === DENIAL_CODES.RULE_DENIED) {
      console.error(error.operation, error.reason)
    } else {
...
3. If no rule addresses the attempted operation, WDK blocks the call with `PolicyViolationError.code: 'NO_APPLICABLE_RULE'`.
...
8. If rules addressed the operation but none matched, WDK blocks with `PolicyViolationError.code: 'GOVERNED_BUT_UNMATCHED'`.
...
Default-deny evaluation already existed before beta.17. This release expands which methods reach it. In beta.18, a call that no rule addresses fails with `code: 'NO_APPLICABLE_RULE'` and identifies the blocked method through `operation`.
...
console.log(result.decision, result.code, result.reason, result.trace)
```

Simulation results include `decision`, `code`, `policy_id`, `matched_rule`, `reason`, and `trace`. `code` is `null` on `ALLOW`, `RULE_DENIED` when a `DENY` rule matched, `NO_APPLICABLE_RULE` when no rule addressed the operation, or `GOVERNED_BUT_UNMATCHED` when addressed rules did not match. Protocol write methods are also mirrored, for example `account.simulate.getSwapProtocol(label).swap(...)` or `account.simulate.getSdaProtocol(label).createDepositAddress(...)`.
...
`PolicyViolationError` means a governed call was blocked by a matching `DENY` rule or by default-deny when no `ALLOW` rule matched. Its `operation` names the blocked method. Branch on its stable `code` value rather than parsing `reason`, because `RULE_DENIED` can carry application-owned rule text. The exported `DENIAL_CODES` object provides the three values used by errors and simulations.
...
- Use [Protocol Integration](/sdk/core-module/guides/protocol-integration) for protocol registration and method setup.
