...
Use discovery results to build token and chain selectors, but continue to show the quote details before execution. `executeSwidge()` is the only write step in the shared swidge flow.
