...
Omitting `signUrl` returns unsigned widget URLs. For signed URLs, use the backend callback pattern in [Configuration](/sdk/fiat-modules/fiat-moonpay/configuration); returning the input unchanged does not sign it.
...
Treat `metadata` as potentially sensitive provider data. Avoid logging or retaining the complete object when the application needs only the normalized status and asset fields.
