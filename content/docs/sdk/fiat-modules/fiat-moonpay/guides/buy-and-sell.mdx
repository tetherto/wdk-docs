...
You can build a purchase URL with [`buy()`](/sdk/fiat-modules/fiat-moonpay/api-reference) when you know the fiat spend. The URL is signed only when the protocol was configured with a `signUrl` callback:
