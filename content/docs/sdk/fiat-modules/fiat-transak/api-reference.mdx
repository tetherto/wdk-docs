...
<Callout type="warn">
The published `1.0.2` types make `options.config` optional, but Transak's Create Widget URL API requires `referrerDomain` for both `buy()` and `sell()`. Include it at runtime even though TypeScript does not enforce it.
</Callout>
...
interface TransakBuyParams {
  ...
  walletAddress?: string;
  walletAddressesData?: TransakWalletAddressesData; // skipped if walletAddress is passed
...
### `TransakWalletAddressesData`

Wallet addresses for pre-filling multiple destination addresses at once, keyed by Transak network identifier and/or coin symbol.

```typescript
interface TransakWalletAddressesData {
  networks?: Record<string, TransakWalletAddressEntry>; // keyed by network identifier, e.g. 'ethereum'
  coins?: Record<string, TransakWalletAddressEntry>; // keyed by coin symbol, e.g. 'BTC'
}

interface TransakWalletAddressEntry {
  address: string;
  addressAdditionalData?: string; // e.g. a memo/tag, when applicable
}
```
...
interface TransakUserData {
  ...
  dob?: string; // ISO 8601 date, e.g. '1994-08-26'
  address?: TransakUserAddress;
}

interface TransakUserAddress {
  addressLine1?: string;
  addressLine2?: string;
  city?: string;
  state?: string;
...
interface TransakOrder {
  id: string; // Published 1.0.2 type; current provider examples return `_id`
...
The published `1.0.2` declaration names the order identifier `id`, while Transak's current Get Order response documentation uses `_id`. Until that package/provider mismatch is reconciled, persist the `txId` passed to `getTransactionDetail()` and do not depend on `metadata.id` at runtime.
