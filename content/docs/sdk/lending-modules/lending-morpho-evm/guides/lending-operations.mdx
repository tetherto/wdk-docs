...
Create `walletClient` with `viem` using the same signer and address as the WDK EVM account. Borrow requirements can be authorization transactions or, when `supportSignature: true` is enabled, signable authorization requests. Send only transaction requirements; sign a request and pass the resulting signature to `borrow()`:
