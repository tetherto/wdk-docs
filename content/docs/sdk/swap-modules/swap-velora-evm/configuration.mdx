...
Velora beta.8 cannot use accounts derived by `WalletManagerEvm` beta.19: their shared ethers provider is not a URL or EIP-1193 input. Use the direct `WalletAccountEvm` setup above with its original provider value. Direct accounts do not receive WDK Core policies or middleware, so retain required application checks before approval and execution.

For a compatible wallet version, policy-governed integrations use [`wdk.registerProtocol()`](/sdk/core-module/api-reference#registerprotocolblockchain-label-protocol-config) and the account's protocol getter. That registration path does not fix the beta.19 incompatibility. Do not pass a governed account proxy directly to this constructor: the proxy hides `_config`.
