...
| **Any MCP-compatible agent** | Use [WDK CLI](/ai/wdk-cli/mcp/) for a ready-made local daemon, or the [MCP Toolkit](/ai/mcp-toolkit/) for a custom MCP server |
| **Any local CLI-capable agent** | Use [WDK CLI](/ai/wdk-cli/) with `--json` output |
...
- [WDK CLI](/ai/wdk-cli/) - Local CLI, daemon, and MCP server for AI agents
