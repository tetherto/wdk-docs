...
| --- | --- | --- |
| [`@tetherto/wdk-wallet-evm`](/sdk/wallet-modules/wallet-evm) | Configured built-in EVM chains | Ethereum + Arbitrum enabled |
...
Use the package's `CHAINS` export as the complete accepted-identifier registry for the installed release; see [Built-in Chain Identifiers](/ai/mcp-toolkit#built-in-chain-identifiers).
...
| `WDK_CHAINS` | No | `ethereum,arbitrum,bitcoin` | Comma-separated identifiers from the package's `CHAINS` export |
