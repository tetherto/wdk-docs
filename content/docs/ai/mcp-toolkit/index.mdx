...
## Built-in Chain Identifiers

The pinned release exports identifiers for 13 built-in chains. Common identifiers used across these docs include:
...
<Callout type="info">
Read the package's `CHAINS` export when your application needs the complete pinned registry. You can register **any** blockchain name - the constants are for convenience only. For custom chains, register tokens manually with `registerToken()`.
</Callout>
