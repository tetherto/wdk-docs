...
<Callout type="warn">
Do not enable Velora beta.8, USD₮0 bridge beta.10, or Aave beta.7 with EVM wallet beta.19. These adapters reject the provider on manager-derived accounts when the protocol is constructed, so the example below omits them. Check the installed wallet and adapter versions before registering them; see [EVM provider compatibility](/sdk/wallet-modules/wallet-evm/configuration#provider). A separate direct account does not inherit the server's registered account controls.
</Callout>
