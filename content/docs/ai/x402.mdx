...
x402 with WDK works on any EVM chain where USD₮0 is deployed (see full list at [docs.usdt0.to](https://docs.usdt0.to/technical-documentation/deployments)). However, we recommend **Plasma** and **Stable** for x402 payments. Both chains are purpose-built for USD₮0 transfers with near-instant finality and near-zero fees. Agents only need to hold USD₮0.
...
Before you can make x402 payments, your wallet needs USD₮0 on the target chain. If you hold a supported source token—for example, USD₮ on Ethereum—bridge it using `@tetherto/wdk-protocol-bridge-usdt0-evm`.
...
const amount = 10000000n; // 10 USDt (6 decimals)
...
<Callout type="info">
The module supports multiple source chains. Point your wallet at the source chain's RPC, then resolve the exact token variant and address from [`getSupportedTokens()`](/sdk/bridge-modules/bridge-usdt0-evm/api-reference) or the current [USD₮0 deployments](https://docs.usdt0.to/technical-documentation/deployments). Do not reuse the Ethereum address on another chain.
</Callout>
...
<Callout type="info">
**About the Semantic facilitator:** [Semantic](https://docs.semanticpay.io) operates a public x402 facilitator at `https://x402.semanticpay.io`. It currently verifies and settles USD₮0 payments on Plasma and Stable. This is a third-party service not operated, endorsed, or guaranteed by Tether.
...
const walletAccount = await new WalletManagerEvm(process.env.FACILITATOR_MNEMONIC, {
  provider: process.env.RPC_URL, // Any EVM chain with USDt0
}).getAccount();
...
<Callout type="info">
The facilitator wallet and the seller wallet can use different seed phrases. The facilitator pays gas; the seller receives USD₮0. The facilitator wallet must have enough native token to pay gas.
</Callout>
