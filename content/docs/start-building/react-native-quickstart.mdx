...
function LockButton() {
  ...
  const handleLock = async () => {
    // This does not stop the worklet or confirm disposal completion.
    await lock()
  }
...
When a wallet is active, `lock()` sends a one-way full WDK disposal request and then resets local lifecycle flags. The Bare worklet and IPC connection stay running, and the call does not wait for worklet cleanup acknowledgement. Treat it as an application access boundary, not evidence that every seed or key copy was erased from memory.
...
React Native Core beta.20 does not enforce biometrics and does not accept a `requireBiometrics` provider prop. Run your app's biometric, passcode, or other authentication flow before calling `createWallet()`, `restoreWallet()`, `unlock()`, `switchWallet()`, or key-access methods.
...
**Worklet startup reports an error**

After fixing the reported bundle, configuration, or native startup cause, restart the application so it creates a fresh JavaScript runtime. React Native Core beta.20 has [no supported in-process recovery from a failed worklet start](/tools/react-native-core/api-reference#startup-failure-recovery); remounting `WdkAppProvider` in the same runtime does not reset its readiness state.
...
Beta.20's published `default` and `types` export targets are absent. Configure TypeScript to select the included React Native source export:
...
Use a current React Native or Expo Metro configuration that honors package exports and retains the `react-native` condition. `skipLibCheck` can suppress checks inside declarations after resolution; it cannot repair the missing beta.20 export targets.
