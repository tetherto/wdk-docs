...
<Callout type="warn">
**Beta** - This documentation describes `@tetherto/wdk-cli@1.0.0-beta.3`. Test integrations with a dedicated wallet and limited funds before relying on them.
</Callout>
...
Beta.3 declares its built-in wallet modules as normal package dependencies. It does not use an install lifecycle script, so no `--allow-scripts` option is required.
