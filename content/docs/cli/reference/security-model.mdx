...
| CLI and MCP sends, swaps, and bridges | Dry-run support and caller-level guidance | The daemon does not enforce a second confirmation or passphrase check for an unlocked wallet; route execution requotes and may differ from a preview |
...
`wdk module add` does not register a package as a swap or bridge protocol. Routing protocol entries come from the CLI's installed catalog. Installing a package named by an existing catalog entry still grants that package the same process and wallet access described below.
...
Swap and bridge previews are observations, not execution commitments. Execution requotes the available protocols and can select a different route or amount. Route ranking compares token input or output amounts and excludes separate gas and bridge fees. Review those fees, any included-fee note, and all `skipped` protocols before deciding whether to execute.
...
- preview the recipient, amount, networks, tokens, route, and fees before a send, swap, or bridge
- treat a route preview as stale at execution time because execution requotes
