...
| CLI and MCP sends, swaps, and bridges | Dry-run support, optional provider restriction, and caller-level guidance | The daemon does not enforce a second confirmation or passphrase check for an unlocked wallet; route execution requotes without CLI output, fee, slippage, or expiry limits bound to the preview |
| Message signing | Caller can display the message and request approval | Signing is immediate, with no dry run or daemon-enforced confirmation |
...
Beta.5 provides `wallet import --seed-stdin` and `wallet change-passphrase --new-passphrase-stdin` for human-operated scripts. Standard input avoids command-line arguments, but it is not a secure enclave: the secret still enters JavaScript process memory and can leak through an input file, pipeline, parent process, or automation environment. Do not expose these flags to an agent or untrusted script.
...
- a same-user process that can connect to `daemon.sock` can derive addresses, read balances and transactions, sign messages, request quotes, and execute signed transactions
...
## Custom module, protocol, and method trust

Beta.5 installs built-in wallet modules as normal `@tetherto/wdk-cli` dependencies and does not need a postinstall lifecycle-script allowlist. This does not make `wdk module add` a safe package verifier.

Adding a custom module:

- resolves npm's current version when no version is supplied, then records the resolved exact pin
- accepts and stores an explicit tag or range verbatim; use a reviewed literal version because beta.5 does not enforce exact semver
...
`wdk module add` does not register a package as a swap or bridge protocol, and beta.5 rejects built-in catalog packages. Routing protocol entries and built-in pins come from the CLI release. Repair a missing built-in package by reinstalling the exact pinned CLI version; reserve `module add` for reviewed custom modules.
...
Message signing has the same authorization boundary. CLI `wdk message sign` and MCP `sign_message` sign immediately against an unlocked account. Show the exact wallet, network, account index, and message and require approval before signing.

Swap and bridge previews are observations, not execution commitments. Execution requotes; unless the caller restricts `protocol`, it can select a different provider, and amounts or fees can change even when the provider is fixed. Beta.5 exposes no CLI or MCP output, input, fee, slippage, or expiry limit bound to the preview. Preview output also omits the selected wallet, account index, and resolved recipient; select them explicitly and verify the destination address with `wdk get address` or MCP `get_address` before quoting. Route ranking compares token input or output amounts and excludes separate gas and bridge fees.

Do not use the beta.5 cross-network swap option. Candidate selection can admit a same-chain swap provider, and the adapter then omits the destination network from that provider's calls. A Velora route with an ERC-20 input also requires a prior, bounded allowance to the chain-specific spender returned by Velora SDK 9's [`swap.getSpender()`](https://github.com/VeloraDEX/sdk/blob/86679ae70686ad3f01ee29baba9033be0a1b742b/src/methods/swap/spender.ts#L33-L63) lookup; beta.5 does not expose that lookup or approve automatically. Native-token input has no ERC-20 allowance, and other protocols can manage approval differently.
...
- select the wallet and account index explicitly, verify the destination address separately, and pass it as the recipient before a swap or bridge preview
- treat a route preview as advisory because execution requotes without a minimum output, maximum input, fee, or expiry bound relative to the preview
- inspect the exact message before signing it
...
- treat MCP clients and agents as capable of signing messages and executing real sends, swaps, and bridges
