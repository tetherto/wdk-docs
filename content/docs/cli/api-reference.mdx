---
title: API Reference
description: Complete WDK CLI beta.5 command and option reference
docType: reference
...
Quotes or executes a same-network or cross-network token swap.
...
| `--to-network <network>` | No | Source network | Destination network for a cross-network swap |
| `--recipient <address>` | No | Selected account | Destination-network recipient |
| `--protocol <name>` | No | Best route | Restrict quoting to one protocol in the installed catalog |
| `--wallet <name>` | No | Default wallet | Wallet selection |
| `--index <n>` | No | Configured index, initially `0` | Non-negative account index |
...
<Callout type="error">
Without `--dry-run`, `wdk swap` executes immediately. Execution requests fresh quotes, so its protocol and amounts can differ from an earlier preview. There is no daemon confirmation step.
</Callout>

If `--recipient` is omitted, the daemon derives an address on the destination network from the selected wallet and the same account index. `wdk module add` does not add a protocol to the installed routing catalog. See [Swap and Bridge](/cli/guides/swap-and-bridge) for the review workflow.
...
The source and destination networks must differ. The omitted-recipient behavior, ranking, skipped-protocol reporting, execution requote, and immediate-execution warning are the same as for `wdk swap`.
