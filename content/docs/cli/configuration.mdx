...
Use `--protocol NAME` to restrict a request to one short name in this installed catalog. Omitting the option lets the CLI quote every installed protocol that supports the requested operation.

<Callout type="info">
`wdk module add` can install a package needed by an existing catalog entry, but it does not create a swap or bridge protocol entry. The beta.5 CLI has no command for adding protocol definitions to the routing catalog.
</Callout>
