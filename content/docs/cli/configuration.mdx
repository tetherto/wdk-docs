---
title: Configuration
description: Configure WDK CLI paths, defaults, indexer access, wallet modules, routing protocols, and fiat ramps
docType: reference
...
There are no beta.5 environment-variable mappings for `indexer.baseUrl`, wallet defaults, account defaults, network configuration, custom modules, or MoonPay configuration.
...
| `customModules.PACKAGE.version` | Exact npm version | Created and removed by `wdk module`; do not manage it with `wdk config set` |

Custom-module, custom-network, and custom-token records also live in `config.json`. Manage them with `wdk module`, `wdk network`, and `wdk token` so the CLI can coordinate package installation, validate record shapes, and update related state.
...
Prefer `WDK_INDEXER_API_KEY` when you do not want to persist the indexer key. No environment override is available for MoonPay configuration in beta.5.
...
`config reset --all` preserves the default-wallet selection, custom networks, and custom tokens. It resets the remaining values to their installed defaults, including removing `customModules` registrations. It does not run npm uninstall, so a preserved custom network can keep using a residual package that `wdk module list` no longer tracks until another npm operation prunes it. Record module pins and network specs before a full reset.
...
Changing or resetting a key under `networks` locks all wallets so the next unlock initializes WDK with the new wallet-module configuration. `config reset --all`, `module add`, and `module remove` also lock every running wallet session.

## Module Packages

Beta.5 records built-in module pins in the installed catalog and custom pins under `customModules` in `config.json`.

```bash title="Terminal"
wdk module list
```

Use `wdk module add` and `wdk module remove` instead of editing `customModules` directly. Those commands coordinate the stored pin with npm installation and lock the daemon before its module graph changes.

<Callout type="error">
Custom modules are executable code. Adding one runs npm installation in the CLI package and can run the package's lifecycle scripts. A configured wallet module later runs in the daemon with access to unlocked accounts. Audit and pin the exact artifact before adding it.
</Callout>

Custom packages are installed with npm's `--no-save` option. A later npm operation can prune them. Use `wdk module list` to detect a missing or mismatched package, then run `wdk module add --name PACKAGE` to repair the stored pin. See [Manage Modules](/cli/guides/manage-modules).

## Swap And Bridge Protocols

`wdk swap` and `wdk bridge` discover protocols from the installed CLI package's `wdk.config.json`. Each protocol has a short name, a module package, general configuration, and optional per-network overrides. The CLI passes the general configuration merged with the selected network's override to that protocol module.

Use `--protocol NAME` to restrict a request to one short name in this installed catalog. Beta.5 has no protocol-list command or MCP tool. Read the `protocols` object in its [exact released `wdk.config.json`](https://github.com/tetherto/wdk-cli/blob/09ab893506ca7221bc179cf3702d78958dfd4cd5/wdk.config.json#L62-L78) and compare it with the file shipped in your installation because later catalogs can differ. `wdk module list` reports package names rather than routing keys. Omitting `--protocol` lets the CLI quote the candidates it classifies for the requested operation.

<Callout type="info">
`wdk module add` manages custom modules. It rejects built-in catalog packages and cannot create a swap or bridge protocol entry. Reinstall `@tetherto/wdk-cli@1.0.0-beta.5` to repair a missing built-in package. The beta.5 CLI has no command for adding protocol definitions to the routing catalog.
</Callout>

See [Swap and Bridge](/cli/guides/swap-and-bridge) for routing and execution behavior.
...
`WDK_INDEXER_BASE_URL` is not read by beta.5.
...
<Callout type="warn">
Keep the MoonPay secret key in the signing service. Beta.5 does not send configurable authentication headers to `signUrl`, so bind the service locally, keep it on a private network, or restrict access with network-level controls. Do not expose an unauthenticated public signing endpoint.
</Callout>
