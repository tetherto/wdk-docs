...
There are no beta.3 environment-variable mappings for `indexer.baseUrl`, wallet defaults, account defaults, network configuration, custom modules, or MoonPay configuration.
...
Prefer `WDK_INDEXER_API_KEY` when you do not want to persist the indexer key. No environment override is available for MoonPay configuration in beta.3.
...
Beta.3 records built-in module pins in the installed catalog and custom pins under `customModules` in `config.json`.
...
`WDK_INDEXER_BASE_URL` is not read by beta.3.
...
<Callout type="warn">
Keep the MoonPay secret key in the signing service. Beta.3 does not send configurable authentication headers to `signUrl`, so bind the service locally, keep it on a private network, or restrict access with network-level controls. Do not expose an unauthenticated public signing endpoint.
</Callout>
