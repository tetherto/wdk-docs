...
WDK CLI beta.5 installs its built-in wallet modules as normal dependencies and records their exact versions in its bundled catalog. Use `wdk module` to inspect those pins or to manage a custom package that a custom network needs.
...
If you omit the version, beta.5 resolves npm's current version and pins the resolved value once:
...
<Callout type="warn">
An exact literal is caller discipline, not a beta.5 validation rule. The parser also accepts npm tags and ranges such as `latest` or `^1.2.3` and stores them verbatim. Status uses literal equality, so an installed concrete version then reports `version mismatch`; the daemon prints a warning but still imports and executes that installed package. Do not use a tag or range, and do not treat a warning as a load block.
</Callout>
...
A later npm operation against the global CLI installation can prune a custom package because beta.5 installs custom modules with `--no-save`. If `wdk module list` reports `not installed` or `version mismatch`, run:
...
- Stop if status reports a mismatch; beta.5 still loads mismatched code after warning.
