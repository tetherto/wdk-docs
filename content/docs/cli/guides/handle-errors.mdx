...
This page describes `@tetherto/wdk-cli@1.0.0-beta.5`.
...
The following names are defined by beta.5. A code can appear only on commands that reach the corresponding behavior.
...
The code set is not a closed protocol enum. The daemon and WDK dependencies can pass through additional codes. Beta.5 also recognizes and formats several dependency codes, including `INSUFFICIENT_FUNDS`, `SERVER_ERROR`, and `TIMEOUT`. Consumers should preserve unknown code strings instead of rejecting the response.
...
| `INVALID_ADDRESS` | Confirm the recipient format and selected mainnet or testnet; beta.5 rejects recognized mismatches after the unlock check but before fee estimation or broadcast |
...
| `QUOTE_REJECTED` | Inspect every protocol failure. For a Velora ERC-20 route, follow the [allowance prerequisite](/cli/guides/swap-and-bridge#prepare-a-velora-token-allowance); do not guess the spender or approval amount. Then correct configuration or try a supported pair or amount |
| Built-in protocol package reported missing | Reinstall `@tetherto/wdk-cli@1.0.0-beta.5`; `wdk module add` rejects built-in packages even when the runtime suggestion recommends it |
| Custom module reported `not installed` or `version mismatch` | Run `wdk module add --name PACKAGE` to repair its stored custom pin after reviewing the exact package |
...
- `wdk swap` and `wdk bridge` return successful route previews or executions with unsuccessful protocol quotes listed under `skipped`.

These commands can exit `0` with an incomplete aggregate or route result. If completeness matters, query each required network or token separately and inspect every skipped protocol before acting. A successful beta.5 cross-network swap preview is not proof that its selected provider received the destination network; do not execute that route shape.
