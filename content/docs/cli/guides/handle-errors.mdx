...
| `QUOTE_REJECTED` | Inspect each protocol failure, then correct its configuration or try a supported token pair, network pair, amount, or installed protocol |
...
These commands can exit `0` with an incomplete aggregate or route result. If completeness matters, query each required network or token separately and inspect every skipped protocol before acting.
