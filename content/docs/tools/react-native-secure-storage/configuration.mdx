...
npm install @tetherto/wdk-react-native-secure-storage@1.0.0-beta.4 react-native-keychain expo-crypto expo-local-authentication
```

<Callout type="warn">
The `v1.0.0-beta.5` npm artifact includes the React Native source entry at `src/index.ts`, but it does not include the `dist/` files declared by `main`, `types`, and `exports.default`. The install command pins beta.4, the latest artifact with those compiled JavaScript and declaration files. Use beta.5 only with a React Native resolver configuration you have verified against its source entry.
</Callout>
