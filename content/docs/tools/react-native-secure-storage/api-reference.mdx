...
Pass `requireBiometrics: true` when reading or writing the encryption key to protect it with device authentication. On devices with biometrics enrolled, the library uses `BIOMETRY_ANY_OR_DEVICE_PASSCODE` (biometric prompt with PIN fallback). On PIN-only devices with no biometrics enrolled (Android security level `SECRET`), the library uses `DEVICE_PASSCODE` so the passcode prompt appears correctly.
