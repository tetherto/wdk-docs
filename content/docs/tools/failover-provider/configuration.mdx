...
- Property getters and methods run with the underlying provider as `this`, including providers that use private fields or instance state.
...
- A method result is treated as asynchronous only when its `then` property is a function. Other values, including objects with a non-function `then` property, are returned synchronously.
