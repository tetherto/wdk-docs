...
<Callout type="warn">
If beta.20 reports an `ERROR` before the worklet starts, do not rely on an in-process retry. An asynchronous startup failure can retrigger initialization while the singleton readiness promise remains rejected, and remounting `WdkAppProvider` in the same JavaScript runtime does not recreate that promise. Fix the reported bundle, configuration, or native startup cause, then restart the application so it creates a fresh JavaScript runtime before retrying.
</Callout>
...
| `deleteWallet` | `(walletId: string) => Promise<void>` | Delete stored data, send a one-way full WDK disposal request, and reset local lifecycle state; the worklet and IPC keep running |
| `lock` | `() => Promise<void>` | When a wallet is active, send a one-way full WDK disposal request and clear local wallet state; the worklet and IPC keep running |
...
| `clearTemporaryWallet` | `() => void` | Clear the temporary wallet session. If it is active, also send a one-way full WDK disposal request and clear shared initialization and address state |
| `clearCache` | `() => void` | Clear cached balance data |

Only one wallet can be active. Call and await `lock()` before creating or restoring a different identity. To select a wallet that is already stored, use `switchWallet()` or lock and then unlock that wallet explicitly. `createWallet()`, `restoreWallet()`, `deleteWallet()`, `lock()`, `unlock()`, `switchWallet()`, and `createTemporaryWallet()` use a fail-fast mutex, so an overlapping guarded call rejects instead of queuing. The mutex timeout does not cancel an already-running underlying operation. `switchWallet()` is serialized, but it is not transactional: it locks first and does not restore the previous wallet if the new unlock fails.

<Callout type="error">
Beta.20 protects `deleteWallet()` with the lifecycle mutex; `clearTemporaryWallet()` remains unguarded. Clearing the active temporary wallet resets shared worklet lifecycle state and sends the same one-way full disposal request as `lock()`. Serialize `clearTemporaryWallet()` with lifecycle operations in the app to avoid racing a guarded call. Deleting even an inactive wallet deletes its secure-storage records and then resets shared worklet lifecycle state. A storage failure can leave a partial deletion and prevent the reset from running. Authenticate and explicitly confirm destructive deletion, verify another recovery path first, and verify the stored wallet inventory afterward.
</Callout>

<Callout type="error">
When a wallet is active, `lock()` sends `hrpc.dispose({})` before clearing local state. The reset performed after a successful `deleteWallet()` also sends that request. HRPC disposal is one-way, so neither method waits for worklet cleanup or confirms that every copy was overwritten. The Bare worklet and IPC connection keep running. Treat these methods as application-state boundaries, not zeroization proof.
</Callout>

<Callout type="warn">
React Native Core beta.20 does not enforce biometrics before wallet lifecycle or key-access methods. Authenticate in the app before invoking these methods. For app-background locking, close the authenticated UI boundary immediately and retry `lock()` after any in-flight guarded operation settles; a mutex rejection does not queue or cancel that operation.
</Callout>

<Callout type="warn">
A rejected create, restore, unlock, or temporary-wallet initialization is not cleanup confirmation. If the worklet constructs WDK and a later wallet or protocol registration fails, it can retain a partial instance and its seed buffer. Beta.20 exposes no public worklet teardown method. Do not retry a sensitive flow in the same session on the assumption that rejection removed that state; restart the application or hosting environment so it creates a new worklet before retrying.
</Callout>
...
| Property | Type | Description |
|----------|------|-------------|
| `getEncryptionKey` | `(walletId: string) => Promise<string \| null>` | Get the selected wallet's base64 encryption key |
| `getEncryptedSeed` | `(walletId: string) => Promise<string \| null>` | Get the base64 encrypted seed from storage |
| `getEncryptedEntropy` | `(walletId: string) => Promise<string \| null>` | Get the base64 encrypted entropy from storage |
| `generateEntropyAndEncrypt` | `(wordCount?: 12 \| 24) => Promise<{ encryptionKey: string, encryptedSeedBuffer: string, encryptedEntropyBuffer: string }>` | Generate and encrypt entropy for wallet creation; all returned strings are base64 |
| `getMnemonicFromEntropy` | `(encryptedEntropy: string, encryptionKey: string) => Promise<{ mnemonic: string }>` | Derive a mnemonic from base64 encrypted entropy and key strings |
| `getSeedAndEntropyFromMnemonic` | `(mnemonic: string) => Promise<{ encryptionKey: string, encryptedSeedBuffer: string, encryptedEntropyBuffer: string }>` | Get encrypted seed and entropy from a mnemonic phrase; all returned strings are base64 |

Beta.20 converts these base64 strings to native buffers only at its internal HRPC boundary and overwrites several owned temporary buffers after use. Public hook results and storage values remain immutable JavaScript strings, and conversion can create additional copies. Discard references promptly and do not treat method completion as proof that every copy was erased.
