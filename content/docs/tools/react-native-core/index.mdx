...
If startup reaches `ERROR` before the worklet starts, follow the [beta.20 startup failure recovery guidance](/tools/react-native-core/api-reference#startup-failure-recovery). Remounting `WdkAppProvider` in the same JavaScript runtime is not a recovery path.
