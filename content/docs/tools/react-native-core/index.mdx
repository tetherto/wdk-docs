...
```bash
npm install @tetherto/wdk-react-native-core react-native-bare-kit
```

Starting in beta.14, `react-native-bare-kit` is a peer dependency with a supported range of `>=0.14.5`, so the app must install it explicitly.
