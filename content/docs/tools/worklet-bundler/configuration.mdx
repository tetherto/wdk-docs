...
npm install @tetherto/pear-wrk-wdk@1.0.0-beta.15
npm install --save-dev @tetherto/wdk-worklet-bundler@1.0.0-beta.14
```

Worklet Bundler beta.14 generates entrypoints that import Pear Worklet. These examples pair it with Pear Worklet beta.15 for native-buffer HRPC secret fields, generic-module calls and events, retained-seed cleanup, and the diagnostic export used by `options.handleLeakCheck`. The bundler checks whether the runtime package is installed; it does not enforce this version pairing.
...
Beta.14 generates these modules for both transports. With Pear Worklet beta.15, JSON-RPC initialization constructs them from the encrypted seed pair and matching runtime `modules` config. Module calls retain JSON-string `args`; results and event payloads are decoded JSON values. See the [Pear Worklet JSON-RPC examples](/tools/pear-wrk-wdk/configuration#json-rpc-transport).

### `transport` (optional)

Choose `hrpc` or `jsonrpc`. HRPC is the default and is the transport used by React Native Core. JSON-RPC produces a length-prefixed bundle for a native host and enables addon linking by default. In beta.14, ESM-to-CJS conversion is an independent, explicit option for both transports.
...
- `types`: Declared in the beta.14 config type, but the generator does not honor a custom value and always writes declarations to `./.wdk/index.d.ts`.
...
- `minify` (`boolean`): Declared in the beta.14 config type but not read by the bundle path. ESM-to-CJS conversion minifies independently of this field.
- `sourceMaps` (`boolean`): Declared in the beta.14 config type but not read by the bundle path, so it does not produce source maps.
...
- `platforms` (`('ios' | 'macos' | 'android')[]`): Addon platforms. Defaults to all three when addon linking is active. The beta.14 CLI rejects any unsupported value supplied through `--platforms` instead of silently dropping it.
...
<Callout type="warn">
Beta.14 emits this diagnostic only for HRPC. Setting `options.handleLeakCheck` in a JSON-RPC config passes validation but does not emit diagnostic registration. Custom JSON-RPC entrypoints can register the [Pear Worklet helper](/tools/pear-wrk-wdk/configuration#inspect-suspend-delays) themselves.
</Callout>
...
In the normal `generate` or `generateBundle()` path, beta.14 uses one flag for both build-time conversion and the generated runtime loader:
...
There is no beta.14 CLI flag that overrides this setting. Set `options.convertEsmToCjs` in the config used for the build.
...
Worklet Bundler beta.14 separates required peers from peers explicitly marked optional through `peerDependenciesMeta`:
...
`generate --install` can auto-install missing configured wallet, protocol, generic, preload, and Pear Worklet runtime dependencies after the package manager is detected from the project root. In beta.14, the Pear Worklet runtime is included for both HRPC and JSON-RPC, so the generated entrypoint's runtime dependency is surfaced during validation and installation instead of failing later in `bare-pack`.
...
The `generate` command also accepts `--transport hrpc|jsonrpc`, `--link-addons`, `--skip-link-addons`, `--platforms ios,macos,android`, and `--no-defer-optional-peers`. Beta.14 exits with status `1` when `--platforms` contains a value other than `ios`, `macos`, or `android`; it reports every invalid value instead of continuing with the remaining entries. Set `transport` in `wdk.config.js` when you rely on its transport-specific default bundle path. The `--transport` override is applied after output paths are resolved, so it does not switch `.js` HRPC output to the extensionless JSON-RPC default, or vice versa. If you use the flag, set `output.bundle` explicitly to the intended path.
...
Generated HRPC and JSON-RPC entrypoints handle Bare `suspend` and `resume` with the same lifecycle code. At each event, beta.14 scans the public `require.cache` for every loaded copy of `bare-http1` and `bare-https`, deduplicates shared `globalAgent` objects, and invokes `suspend()` or `resume()` on each agent. It also invokes the configured generic-module runtime's `suspendAll()` or `resumeAll()` method.

These calls are fire-and-forget. Synchronous throws and rejected promises are logged separately for each resource, so one failure does not stop the remaining lifecycle calls. The Bare event is not delayed until asynchronous cleanup completes. Pear Worklet closes modules during full disposal or reinitialization; selective JSON-RPC disposal and `resetWdkWallets()` leave them running.

The lifecycle hooks require no diagnostic flag. `options.handleLeakCheck` remains HRPC-only and only adds handle reporting.
...
- When `bare-pack` reports an unresolved package subpath, beta.14 reduces scoped or unscoped subpaths to the installable package root and prints an install command for the package manager detected from the project root. It prints no install hint when the reported specifier is not an installable package, such as a relative or absolute path, private import, URL or builtin-prefixed specifier, or bare scope.
