...
Generated HRPC and JSON-RPC entrypoints forward Bare `suspend` and `resume` events to loaded HTTP agents and configured generic modules. No diagnostic flag is needed; `options.handleLeakCheck` remains HRPC-only and adds handle reporting.

The hooks log failures and continue with the other resources, but they do not wait for asynchronous cleanup. Treat suspension separately from [disposing the worklet's WDK instance](/tools/pear-wrk-wdk/configuration#dispose-wdk). Full disposal or reinitialization closes generic modules; selective JSON-RPC disposal and `resetWdkWallets()` leave them running.

See the [entrypoint API reference](/tools/worklet-bundler/api-reference#generateentrypointconfig-outputdir) for agent discovery and callback behavior.
