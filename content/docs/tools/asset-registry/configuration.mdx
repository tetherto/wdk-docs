...
Symbol and chain lookups are case-insensitive by default. Address lookups are case-sensitive by default in `v1.0.0-beta.2`; pass `{ caseSensitive: false }` only when the registry and lookup input may use different casing.
...
const byAddressIgnoringCase = registry.getTokenByAddress(
