...
This page explains how to configure a beta.15 worklet context, initialize and dispose WDK, call wallet and generic-module methods, and configure logging and suspend diagnostics. Use the table of contents to find each task.
...
<Callout type="warn">
These maps are not default-deny. List every dynamic surface exposed to an untrusted host. The beta.15 runtime reports denied calls with `METHOD_NOT_ALLOWED`, but the published error-code declaration still omits that member; handle the literal runtime code until the declaration is corrected.
</Callout>
...
JSON-RPC generic-module support was introduced in Pear Worklet beta.13. Supply the same `moduleManagers`, `allowedModuleMethods`, and runtime `modules` config used by HRPC. For generated entrypoints, use [Worklet Bundler beta.14 with Pear Worklet beta.15](/tools/worklet-bundler/configuration#install-the-packages).

## Secret Payload Representations

Beta.15 uses different secret representations at each public boundary:

| Boundary | `encryptionKey` and encrypted seed or entropy fields |
| --- | --- |
| Pear HRPC requests and responses | Native `Buffer` values |
| Framed JSON-RPC requests and responses | Base64 strings |
| React Native Core public wallet hooks | Base64 strings |

Decode values read from string storage before sending them over HRPC. The caller owns its input buffers and every buffer returned by HRPC, and should overwrite them after the final consumer finishes. JSON-RPC converts known secret fields between base64 strings and buffers inside the server. JavaScript strings and additional copies retained by application code cannot be overwritten reliably.
...
Pass both seed fields whenever initialization must create generic modules. If wallet or protocol registration fails after WDK is created, explicitly dispose the partial instance before abandoning or retrying initialization.
...
The runtime manages the decrypted buffer through `context.wdkSeedBuffer`; do not set this field yourself. If decryption or WDK construction fails, beta.15 overwrites temporary buffers owned by that handler. A later wallet or protocol registration failure retains the partial instance and seed until disposal.
...
A full disposal closes generic modules, disposes WDK, and overwrites and releases its retained seed buffer. Once module shutdown completes, the instance and buffer are cleared even if `wdk.dispose()` throws. HRPC `dispose()` is one-way and returns `void`; its return does not acknowledge cleanup completion.

<Callout type="error">
The beta.15 HRPC schema encodes `dispose()` as an empty request. The published `DisposeRequest` type still declares `blockchains`, but HRPC drops that field and performs full disposal. Do not pass `blockchains` to HRPC expecting a selective result. Use `resetWdkWallets()` below when the selected wallets must be disposed and then re-registered.
</Callout>

Framed JSON-RPC does preserve a non-empty `params.blockchains` array for selective disposal. That path retains the WDK instance, seed buffer, and generic modules and returns `{ status: 'disposed' }` after the handler completes. Omitting `blockchains` or sending an empty array requests full disposal.
...
In beta.15, a module method that returns `undefined` or `null` produces the JSON string `"null"` in the HRPC response. JSON-RPC decodes it to `response.result.result === null`. This normalization applies to `callModule()`, not wallet or protocol `callMethod()` results.
...
JSON-RPC supports generic-module initialization, `callModule`, `allowedModuleMethods`, and `moduleEvent` notifications alongside wallet and protocol operations. `resetWdkWallets` remains HRPC-only. Secret-bearing fields use base64 strings on this JSON boundary; the server decodes them before dispatch and encodes buffer results before replying. See the [API reference](/tools/pear-wrk-wdk/api-reference#json-rpc-transport) for all methods and response shapes.
...
Set `LOG_LEVEL` in the worklet environment before loading the package. Accepted values are `DEBUG`, `INFO`, `WARN`, `ERROR`, and `NONE`; beta.15 trims whitespace and ignores case. An unrecognized value falls back to `ERROR`. With `LOG_LEVEL` unset or empty, `NODE_ENV=development` selects `DEBUG`; other environments default to `ERROR`.

Beta.15 no longer emits routine JSON-RPC request, response, or handler-registration info logs. Error logs remain. Do not use the removed info lines as readiness or request-completion signals.
