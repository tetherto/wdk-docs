...
The runtime manages the decrypted buffer through `context.wdkSeedBuffer`; do not set this field yourself. If decryption or WDK construction fails, beta.15 overwrites temporary buffers owned by that handler. After the buffer has been handed to a constructed WDK instance, a later wallet or protocol registration failure retains the partial instance and seed. Treat that failure as requiring explicit disposal before abandoning or retrying initialization.
