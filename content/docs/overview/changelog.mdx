...
- **wallet-evm** ([v1.0.0-beta.14](https://github.com/tetherto/wdk-wallet-evm/releases/tag/v1.0.0-beta.14)): Add `transactionMaxFee` for native EVM `sendTransaction()` and `signTransaction()` flows, separate from `transferMaxFee` for token transfers.
