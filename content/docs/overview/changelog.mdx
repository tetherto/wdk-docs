...
**Changes**
- **wallet-evm** ([v1.0.0-beta.19](https://github.com/tetherto/wdk-wallet-evm/releases/tag/v1.0.0-beta.19)): Accept and reuse an existing ethers `Provider` at runtime, and share one provider object across accounts derived by a wallet manager. The published TypeScript declaration still omits ethers `Provider` from the accepted configuration type.
