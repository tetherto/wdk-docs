...
**What's New**
- **swidge-butter** ([v0.2.0](https://www.npmjs.com/package/@butternetwork/wdk-protocol-swidge-butter/v/0.2.0)): Add the community-maintained [Butter Network Swidge module](/sdk/swidge-modules/swidge-butter/) for chain and token discovery, exact-input quotes, execution through compatible WDK accounts, and settlement tracking. Includes quote pinning, fee limits, and EVM approval handling.

**Changes**
- **wallet-evm** ([v1.0.0-beta.19](https://github.com/tetherto/wdk-wallet-evm/releases/tag/v1.0.0-beta.19)): Accept and reuse an existing ethers `Provider` at runtime, and share one provider object across accounts derived by a wallet manager. The published TypeScript declaration still omits ethers `Provider` from the accepted configuration type. Manager-derived accounts are incompatible with Velora beta.8, bridge-usdt0-evm beta.10, and Aave beta.7; see the [direct-account setup and policy limitations](/sdk/core-module/guides/protocol-integration).
- **wallet-solana** ([v1.0.0-beta.15](https://github.com/tetherto/wdk-wallet-solana/releases/tag/v1.0.0-beta.15)): Re-export seven shared WDK error classes from the package root. Replace generic errors for disposed accounts, missing providers, fee caps, and invalid values; existing typed lookup and timeout errors are now importable from this package. Solana RPC and program failures can still use provider-specific errors.
- **react-native-core** ([v1.0.0-beta.20](https://github.com/tetherto/wdk-react-native-core/releases/tag/v1.0.0-beta.20)): Use Pear Worklet beta.15 and `Buffer` values on the internal HRPC boundary while keeping the public hook and secure-storage boundary base64 encoded. Locking and switching now send a one-way full-disposal request while the worklet and IPC remain running. After secure-storage deletion succeeds, `deleteWallet()` sends the same request; it now uses the lifecycle mutex. The npm artifact still omits its declared `dist` entrypoints.

**Fixes**
- **wdk-asset-registry** ([v1.0.0-beta.4](https://github.com/tetherto/wdk-asset-registry/releases/tag/v1.0.0-beta.4)): Correct the bundled Berachain USD₮0 contract metadata to match the official deployment registry.
...
**What's New**
- **wdk-core** ([v1.0.0-beta.18](https://github.com/tetherto/wdk/releases/tag/v1.0.0-beta.18)): Add stable policy-denial codes through the root `DENIAL_CODES` export, `PolicyViolationError.code`, `PolicyViolationError.operation`, and `SimulationResult.code`. Branch on the code instead of application-owned reason text.
- **WDK CLI** ([v1.0.0-beta.5](https://github.com/tetherto/wdk-cli/releases/tag/v1.0.0-beta.5)): Add message signing and verification, transaction lookup with optional finality waiting, and routed swap and bridge commands. Add matching MCP tools, with dry-run enabled by default for MCP swap and bridge requests.
- **wallet-evm-erc-4337** ([v1.0.0-beta.20](https://github.com/tetherto/wdk-wallet-evm-erc-4337/releases/tag/v1.0.0-beta.20)): Add optional bundler and paymaster request headers. Recognize structured AA50 errors first and map prefund failures consistently during fee estimation, UserOperation building, and bundler submission.

**Changes**
- **pear-wrk-wdk** ([v1.0.0-beta.15](https://github.com/tetherto/pear-wrk-wdk/releases/tag/v1.0.0-beta.15)): [Breaking] Move HRPC secret fields and crypto utilities from base64 strings to `Buffer` values while retaining base64 at the JSON-RPC wire boundary. Schema v4 makes HRPC disposal full-only, changes `workletStart` to an empty handshake, removes `generateEncryptionKey()`, and replaces `validateBase64()` with `validateBuffer()`.
- **wdk-asset-registry** ([v1.0.0-beta.3](https://github.com/tetherto/wdk-asset-registry/releases/tag/v1.0.0-beta.3)): Compile and cache the exported Zod schemas with Zod 4.5.4 without changing their intended shapes.
- **worklet-bundler** ([v1.0.0-beta.14](https://github.com/tetherto/wdk-worklet-bundler/releases/tag/v1.0.0-beta.14)): Generate shared suspend and resume handling for HRPC and JSON-RPC, including dynamically loaded HTTP agents and generic modules. Reject unsupported `--platforms` values; lifecycle hooks remain fire-and-forget with isolated error logging, and handle-leak checks remain HRPC-only.

**Fixes**
- **wallet-spark** ([v1.0.0-beta.26](https://github.com/tetherto/wdk-wallet-spark/releases/tag/v1.0.0-beta.26)): Remove the seed from HD-key derivation error context while retaining the `hdkey` field identifier.
- **bridge-usdt0-evm** ([v1.0.0-beta.10](https://github.com/tetherto/wdk-protocol-bridge-usdt0-evm/releases/tag/v1.0.0-beta.10)): Return the ERC-4337 helper's `bridgeFee` in source-chain native base units and validate EVM, Solana, TON, and TRON recipients during quotes and execution. Token-paymaster `fee` and native `bridgeFee` remain different denominations.
...
**What's New**
- **wallet-evm-hinkal** ([v0.0.7](https://www.npmjs.com/package/@hinkal/wdk-wallet-evm-hinkal/v/0.0.7)): Add Hinkal private sends, scheduled-send tracking, and recovery of stuck shielded balances to seed-derived EVM accounts. Version 0.0.7 renames `@hinkal/wdk-wallet-evm` to `@hinkal/wdk-wallet-evm-hinkal`; update the dependency and imports when upgrading.
...
**What's New**
- **wallet-evm-7702-gasless** ([v1.0.0-beta.4](https://github.com/tetherto/wdk-wallet-evm-7702-gasless/releases/tag/v1.0.0-beta.4)): Add normalized `getTransaction()` and `waitForTransaction()` tracking for UserOperation hashes. Enforce `transactionMaxFee` for newly built token-paymaster sends, signatures, and approvals, include EIP-7702 authorization in quotes for undelegated writable accounts, and scope quote-cache entries by sponsorship and paymaster configuration. A previously signed UserOperation bypasses the new fee-cap check when submitted because its token-denominated fee cannot be reconstructed.
- **wallet-evm-erc-4337** ([v1.0.0-beta.16](https://github.com/tetherto/wdk-wallet-evm-erc-4337/releases/tag/v1.0.0-beta.16)): Add normalized `getTransaction()` and `waitForTransaction()` tracking for UserOperation hashes. Inclusion and finality follow the underlying EVM transaction, while success and fee come from the UserOperation receipt.
- **wallet-solana-gasless** ([v1.0.0-beta.4](https://github.com/tetherto/wdk-wallet-solana-gasless/releases/tag/v1.0.0-beta.4)): Add normalized `getTransaction()` and `waitForTransaction()` tracking with Solana processed, confirmed, and finalized status mapping.
- **wallet-spark** ([v1.0.0-beta.23](https://github.com/tetherto/wdk-wallet-spark/releases/tag/v1.0.0-beta.23)): Add normalized `getTransaction()` and `waitForTransaction()` tracking for Spark transfer IDs. Completed transfers are final, expired or returned transfers are dropped, and Spark transaction fees remain zero.
- **wallet-ton-gasless** ([v1.0.0-beta.11](https://github.com/tetherto/wdk-wallet-ton-gasless/releases/tag/v1.0.0-beta.11)): Add normalized `getTransaction()` and `waitForTransaction()` tracking for signed TON message-body hashes through the embedded TON wallet implementation.
- **wallet-tron** ([v1.0.0-beta.11](https://github.com/tetherto/wdk-wallet-tron/releases/tag/v1.0.0-beta.11)): Add normalized `getTransaction()` and `waitForTransaction()` tracking. Transactions without a block remain pending, committed transactions become confirmed, and transactions at or below the current solidified block become final.
- **wallet-tron-gasfree** ([v1.0.0-beta.9](https://github.com/tetherto/wdk-wallet-tron-gasfree/releases/tag/v1.0.0-beta.9)): Add normalized `getTransaction()` and `waitForTransaction()` tracking for GasFree relay IDs, resolving each relay through its native TRON transaction while retaining the relay ID as the normalized hash.
- **WDK CLI** ([v1.0.0-beta.3](https://github.com/tetherto/wdk-cli/releases/tag/v1.0.0-beta.3)): Add `wdk method list/call`, matching MCP tools for declared chain-specific methods, and `wdk module list/add/remove` for custom modules. Built-in modules are now ordinary pinned dependencies, so the global install no longer needs npm `--allow-scripts`. For custom modules, pass a reviewed literal version: beta.3 accepts tags and ranges, and an installed-version mismatch warns but does not block daemon loading. npm inherits the CLI environment during add/remove, so unset `WDK_PASSPHRASE` and use the interactive confirmation prompt.

**Changes**
- **react-native-core** ([v1.0.0-beta.18](https://github.com/tetherto/wdk-react-native-core/releases/tag/v1.0.0-beta.18)): Move `expo-crypto` to a required peer dependency with range `>=55.0.0 <57.0.0`. Select a version inside that range that also matches the app's Expo SDK; if those requirements do not overlap, the release is incompatible with that SDK line. Bare React Native apps must install and configure Expo modules before adding `expo-crypto`. The npm artifact still omits the declared default `dist/index.js` and `dist/index.d.ts` files, so consumers need a resolver that selects the React Native source condition.
...
**What's New**
- **wallet-btc** ([v1.0.0-beta.14](https://github.com/tetherto/wdk-wallet-btc/releases/tag/v1.0.0-beta.14)): Add address-history-scoped normalized `getTransaction()` and `waitForTransaction()` tracking. Transactions become final at six confirmations; the public `IBtcClient` declaration now requires `getBlockHeight()` so custom TypeScript clients can resolve confirmation depth.
- **wallet-evm** ([v1.0.0-beta.17](https://github.com/tetherto/wdk-wallet-evm/releases/tag/v1.0.0-beta.17)): Add normalized `getTransaction()` and `waitForTransaction()` tracking, including reverted receipt results, pending-to-dropped detection when the sender nonce advances, and finalized-block checks where the RPC supports the `finalized` tag. Account-derivation failures now throw `InvalidSignerError` instead of `SignerError`.
- **wallet-solana** ([v1.0.0-beta.13](https://github.com/tetherto/wdk-wallet-solana/releases/tag/v1.0.0-beta.13)): Add normalized `getTransaction()` and `waitForTransaction()` tracking that maps Solana processed, confirmed, and finalized signature states to WDK finality.
- **wallet-ton** ([v1.0.0-beta.13](https://github.com/tetherto/wdk-wallet-ton/releases/tag/v1.0.0-beta.13)): Add normalized `getTransaction()` and `waitForTransaction()` tracking for signed message-body hashes. Receipt lookup now derives the TON Center v3 endpoint from the configured client origin instead of always querying the hard-coded mainnet endpoint. A direct TON client forwards its API key; the failover wrapper does not expose or forward a primary client's key in beta.13.
- **wallet-aptos** ([v1.0.0-beta.2](https://github.com/tetherto/wdk-wallet-aptos/releases/tag/v1.0.0-beta.2)): Add normalized `getTransaction()` and `waitForTransaction()` tracking, export `AptosTransactionInfo`, trim and validate transaction hashes, and map pending transactions to pending while treating committed transactions as final.
- **backup-cloud** ([v1.0.0-beta.1](https://github.com/tetherto/wdk-backup-cloud/releases/tag/v1.0.0-beta.1)): Introduce Google Drive `appDataFolder` and CloudKit private-database storage for an already-encrypted wallet master key, with upload, download, existence, availability, and idempotent delete operations plus typed cloud errors. The package transports an opaque encrypted string; it does not encrypt keys, perform OAuth, or prove end-to-end wallet recovery.

**Changes**
- **react-native-core** ([v1.0.0-beta.17](https://github.com/tetherto/wdk-react-native-core/releases/tag/v1.0.0-beta.17)): [Breaking] Make wallet identity caller-owned and lifecycle operations explicit. Remove automatic initialization, provider-managed background locking, `currentUserId`, `retry`, and `setActiveWalletId`; require `unlock(walletId)`, make `lock()` asynchronous, add mutex-protected `switchWallet(walletId)`, and make `LOCKED.walletId` optional. Switching locks the previous wallet before attempting the new unlock and does not roll back if that unlock fails. Address and balance requests now reject stale writes across wallet changes. Applications remain responsible for biometric or other authentication policy.

**Fixes**
- **wallet-solana-gasless** ([v1.0.0-beta.3](https://github.com/tetherto/wdk-wallet-solana-gasless/releases/tag/v1.0.0-beta.3)): Bind the quoted paymaster fee to the decoded SPL payment instruction and require its destination to match the configured paymaster associated-token account before signing or sending.
- **wallet-ton-gasless** ([v1.0.0-beta.10](https://github.com/tetherto/wdk-wallet-ton-gasless/releases/tag/v1.0.0-beta.10)): Validate relay estimates before quote, sign, or send, including the original Jetton transfer fields, sender and relay, future validity, positive commission, exact two-message shape, and paymaster commission payload.
- **wallet-tron** ([v1.0.0-beta.10](https://github.com/tetherto/wdk-wallet-tron/releases/tag/v1.0.0-beta.10)): Verify that an unsigned prebuilt TronWeb transaction's `txID` matches its raw transaction data before wallet signing, then apply the existing owner check. Already-signed direct-broadcast inputs remain outside this signing guard.
...
**Changes**
- **swidge-moonpay-trade** ([v0.2.0](https://www.npmjs.com/package/@moonpay/wdk-protocol-swidge-moonpay-trade/v/0.2.0)): [Breaking] Execute source deposits through concrete writable WDK accounts, with minimum-output checks and deposit-gas fee reporting.
...
**Fixes**
- **p2p-address-book** ([v1.0.0-beta.3](https://github.com/tetherto/wdk-p2p-address-book/releases/tag/v1.0.0-beta.3)): Fix `suspend()` and `resume()` to pause and restore blind peering, an active swarm, and the book's Corestore. Local-only books now suspend and resume without requiring a swarm.
- **wallet-btc** ([v1.0.0-beta.13](https://github.com/tetherto/wdk-wallet-btc/releases/tag/v1.0.0-beta.13)): Make Blockbook balance reads subtract address-owned confirmed inputs spent by pending transactions, credit address-owned pending change only for trusted transaction chains, and exclude other unconfirmed incoming funds. Electrum and custom clients that do not report `unconfirmedOutgoing` still use their raw net unconfirmed balance, so provider results can differ temporarily.
...
**Changes**
- **worklet-bundler** ([v1.0.0-beta.9](https://github.com/tetherto/wdk-worklet-bundler/releases/tag/v1.0.0-beta.9)): Publish the beta.8 method-allowlist configuration to npm and add generated HRPC logs for Bare `suspend`, `resume`, and `idle` lifecycle events.
...
**What's New**
- **wdk-wallet** ([v1.0.0-beta.16](https://github.com/tetherto/wdk-wallet/releases/tag/v1.0.0-beta.16)): Add the `@tetherto/wdk-wallet/multisig` entrypoint with read-only, writable, and owner-management interface classes and proposal, message, threshold, and auto-execution result types. These are implementation contracts whose base methods throw `NotImplementedError`; wallet modules must provide the chain-specific behavior.
...
**What's New**
- **worklet-bundler** ([v1.0.0-beta.8](https://github.com/tetherto/wdk-worklet-bundler/releases/tag/v1.0.0-beta.8)): Add opt-in `allowedMethods` restrictions for network and nested protocol calls and HRPC-only `allowedModuleMethods` restrictions for generic modules. This GitHub release has no matching public npm artifact; install beta.9 for this surface.
...
**What's New**
- **create-wdk-module**: Added documentation for the [`create-wdk-module`](/tools/create-wdk-module) CLI scaffolding tool. Updated the [Community Modules](https://wdk.tether.io/developers/blocks) and [SDK Get Started](/sdk/get-started) pages with references to the new tool.
...
- **[Community Modules](https://wdk.tether.io/developers/blocks)**: Added [`@base58-io/wdk-wallet-cosmos`](https://github.com/base58-io/wdk-wallet-cosmos) — wallet module for Cosmos-compatible blockchains by [Base58](https://base58.io/).
...
**What's New**
- **[x402 Payments](/ai/x402)**: New guide for accepting and making instant USD₮0 payments over HTTP using WDK self-custodial wallets. Covers the x402 protocol, buyer integration with `@tetherto/wdk-wallet-evm`, seller setup with hosted and self-hosted facilitators, and bridging USD₮ into USD₮0 on Plasma and Stable.
...
- **indexer-api**:
  ...
  - Added XAU₮0 support for Plasma (API token key: `xaut`)
...
- Added a [Community Modules](https://wdk.tether.io/developers/blocks) section to highlight community-built modules
...
- Introduced an [All Modules](https://wdk.tether.io/developers/blocks) page in docs for comprehensive module listings
