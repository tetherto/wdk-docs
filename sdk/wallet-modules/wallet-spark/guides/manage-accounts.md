...
This guide explains how to [retrieve accounts by index](#retrieve-accounts-by-index) and [iterate over accounts](#iterate-over-accounts).
...
{% hint style="warning" %}
Custom derivation paths are not supported on Spark. The Spark wallet manager does not support `getAccountByPath`; use indexed accounts only. See [WalletManagerSpark](../api-reference.md#walletmanagerspark) in the API reference.
{% endhint %}
