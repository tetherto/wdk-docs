...
- Swidge, swap, bridge, lending, fiat, and pricing/data providers use their
  functional folders; cross-list community packages through `content/feeds/all-modules.md`.
...
- Catalog-only integrations edit `content/feeds/all-modules.md`; coordinate the
  external catalog separately and do not recreate retired routes.
...
- Catalog-only integration: verified `content/feeds/all-modules.md` row and public destination; no empty local page set.
...
For `CATALOG_ONLY`, allow only `content/feeds/all-modules.md` and run the repository link checker with `LINK_CHECK_EXTERNAL=false`.
...
If the feed changed, run `npm run sync:all-modules-feed` and inspect
`public/llms-full.txt`. If docs-site content or navigation changed, run:
